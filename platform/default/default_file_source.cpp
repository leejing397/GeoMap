...
public:
    Impl(ActorRef<Impl> self, std::shared_ptr<FileSource> assetFileSource_, const std::string& cachePath, uint64_t maximumCacheSize)
        ...
        self.invoke(&Impl::initializeMBTileFileSource);
        // 初始化MBTileFileSource
//
    }

    void initializeMBTileFileSource(){
        mbTileFileSource =  std::make_unique<MBTileFileSource>();
    }
    ...
    void request(AsyncRequest* req, Resource resource, ActorRef<FileSourceRequest> ref) {
        ...
        } else if(MBTileFileSource::acceptsURL(resource.url)){   //wsj 2017-11-1
            tasks[req] =  mbTileFileSource->request(resource,callback);
        } else{
...
private:
    ...
    std::unique_ptr<MBTileFileSource> mbTileFileSource;
