...
@implementation GCLSDKUpdateChecker

+ (void)checkForUpdates {
#if TARGET_IPHONE_SIMULATOR
    // Abort if running in a playground.
    if ([[NSBundle mainBundle].bundleIdentifier hasPrefix:@"com.apple.dt.playground."] ||
        NSProcessInfo.processInfo.gcl_isInterfaceBuilderDesignablesAgent) {
        return;
    }

    NSString *currentVersion = [NSBundle gcl_frameworkInfoDictionary][@"MGLSemanticVersionString"];
