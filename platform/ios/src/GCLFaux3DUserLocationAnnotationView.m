...
#import "GCLUserLocationHeadingArrowLayer.h"
#import "GCLUserLocationHeadingBeamLayer.h"

const CGFloat GCLUserLocationAnnotationDotSize = 22.0;
const CGFloat GCLUserLocationAnnotationHaloSize = 115.0;

const CGFloat GCLUserLocationAnnotationPuckSize = 45.0;
const CGFloat GCLUserLocationAnnotationArrowSize = GCLUserLocationAnnotationPuckSize * 0.5;

const CGFloat GCLUserLocationHeadingUpdateThreshold = 0.01;

@implementation GCLFaux3DUserLocationAnnotationView
{
    ...
    CALayer<GCLUserLocationHeadingIndicator> *_headingIndicatorLayer;
...
- (void)update
{
    ...
    {
        CGFloat frameSize = (self.mapView.userTrackingMode == GCLUserTrackingModeFollowWithCourse) ? GCLUserLocationAnnotationPuckSize : GCLUserLocationAnnotationDotSize;
        [self updateFrameWithSize:frameSize];
    }

    if (CLLocationCoordinate2DIsValid(self.userLocation.coordinate))
    {
        (self.mapView.userTrackingMode == GCLUserTrackingModeFollowWithCourse) ? [self drawPuck] : [self drawDot];
        [self updatePitch];
    }
...
- (void)updatePitch
{
    if (self.mapView.camera.pitch != _oldPitch)
    {
        ...
        CATransform3D t = CATransform3DRotate(CATransform3DIdentity, GCLRadiansFromDegrees(self.mapView.camera.pitch), 1.0, 0, 0);
...
{
    CGFloat pitch = GCLRadiansFromDegrees(self.mapView.camera.pitch);
...
- (void)drawPuck
{
    if ( ! _puckModeActivated)
    {
        ...
        [self updateFrameWithSize:GCLUserLocationAnnotationPuckSize];
    }
    ...
    if ( ! _puckDot)
    {
        _puckDot = [self circleLayerWithSize:GCLUserLocationAnnotationPuckSize];
    ...
    if ( ! _puckArrow)
    {
        ...
        _puckArrow.bounds = CGRectMake(0, 0, round(GCLUserLocationAnnotationArrowSize), round(GCLUserLocationAnnotationArrowSize));
    ...
    {
        _puckArrow.affineTransform = CGAffineTransformRotate(CGAffineTransformIdentity, -GCLRadiansFromDegrees(self.mapView.direction - self.userLocation.location.course));
    }
...
{
    CGFloat max = GCLUserLocationAnnotationArrowSize;
...
- (void)drawDot
{
    if (_puckModeActivated)
    {
        ...
        [self updateFrameWithSize:GCLUserLocationAnnotationDotSize];
    }

    // heading indicator (tinted, beam or arrow)
    //
    BOOL headingTrackingModeEnabled = self.mapView.userTrackingMode == GCLUserTrackingModeFollowWithHeading;
    ...
    if (showHeadingIndicator)
    {
        ...
        if (([_headingIndicatorLayer isMemberOfClass:[GCLUserLocationHeadingBeamLayer class]] && ! headingTrackingModeEnabled) ||
            ([_headingIndicatorLayer isMemberOfClass:[GCLUserLocationHeadingArrowLayer class]] && headingTrackingModeEnabled))
        {
        ...
        if ( ! _headingIndicatorLayer && headingAccuracy)
        {
            ...
            {
                _headingIndicatorLayer = [[GCLUserLocationHeadingBeamLayer alloc] initWithUserLocationAnnotationView:self];
                [self.layer insertSublayer:_headingIndicatorLayer below:_dotBorderLayer];
            }
            else
            {
                _headingIndicatorLayer = [[GCLUserLocationHeadingArrowLayer alloc] initWithUserLocationAnnotationView:self];
        ...
        {
            CGFloat rotation = -GCLRadiansFromDegrees(self.mapView.direction - self.userLocation.heading.trueHeading);

            // Don't rotate if the change is imperceptible.
            if (fabs(rotation) > GCLUserLocationHeadingUpdateThreshold)
            {
                [CATransaction begin];
                [CATransaction setDisableActions:YES];
    ...
    if (_accuracyRingLayer && (_oldZoom != self.mapView.zoomLevel || _oldHorizontalAccuracy != self.userLocation.location.horizontalAccuracy))
    {
        ...
        if (accuracyRingSize > GCLUserLocationAnnotationDotSize + 15)
        {
        ...
        else
        {
            ...
            _haloLayer.bounds = CGRectMake(0, 0, GCLUserLocationAnnotationHaloSize, GCLUserLocationAnnotationHaloSize);
            _haloLayer.cornerRadius = GCLUserLocationAnnotationHaloSize / 2.0;
    ...
    if ( ! _haloLayer)
    {
        _haloLayer = [self circleLayerWithSize:GCLUserLocationAnnotationHaloSize];
    ...
    if ( ! _dotBorderLayer)
    {
        _dotBorderLayer = [self circleLayerWithSize:GCLUserLocationAnnotationDotSize];
    ...
    if ( ! _dotLayer)
    {
        _dotLayer = [self circleLayerWithSize:GCLUserLocationAnnotationDotSize * 0.75];
