...
#import "UIViewController+GCLAdditions.h"

#import "GCLFaux3DUserLocationAnnotationView.h"
#import "GCLUserLocationAnnotationView.h"
#import "GCLUserLocationAnnotationView_Private.h"
#import "GCLUserLocation_Private.h"
#import "GCLAnnotationImage_Private.h"
#import "GCLAnnotationView_Private.h"
#import "GCLScaleBar.h"
#import "GCLStyle_Private.h"
#import "GCLStyleLayer_Private.h"
#import "GCLMapboxEvents.h"
#import "GCLSDKUpdateChecker.h"
#import "GCLCompactCalloutView.h"
#import "GCLAnnotationContainerView.h"
#import "GCLAnnotationContainerView_Private.h"
#import "GCLAttributionInfo_Private.h"
#import "GCLMapAccessibilityElement.h"
...
class MBGLView;
class GCLAnnotationContext;

const CGFloat GCLMapViewDecelerationRateNormal = UIScrollViewDecelerationRateNormal;
const CGFloat GCLMapViewDecelerationRateFast = UIScrollViewDecelerationRateFast;
const CGFloat GCLMapViewDecelerationRateImmediate = 0.0;

/// Indicates the manner in which the map view is tracking the user location.
typedef NS_ENUM(NSUInteger, GCLUserTrackingState) {
    /// The map view is not yet tracking the user location.
    GCLUserTrackingStatePossible = 0,
    /// The map view has begun to move to the first reported user location.
    GCLUserTrackingStateBegan,
    /// The map view begins a significant transition.
    GCLUserTrackingStateBeginSignificantTransition,
    /// The map view has finished moving to the first reported user location.
    GCLUserTrackingStateChanged,
};

const NSTimeInterval GCLAnimationDuration = 0.3;
...
const NSTimeInterval GCLUserLocationAnimationDuration = 1.0;

/// Distance between the map view’s edge and that of the user location
/// annotation view.
const UIEdgeInsets GCLUserLocationAnnotationViewInset = UIEdgeInsetsMake(50, 0, 50, 0);

const CGSize GCLAnnotationUpdateViewportOutset = {150, 150};
const CGFloat GCLMinimumZoom = 3;

/// Minimum initial zoom level when entering user tracking mode.
const double GCLMinimumZoomLevelForUserTracking = 10.5;

/// Initial zoom level when entering user tracking mode from a low zoom level.
const double GCLDefaultZoomLevelForUserTracking = 14.0;

const NSUInteger GCLTargetFrameInterval = 1;  // Target FPS will be 60 divided by this value

/// Tolerance for snapping to true north, measured in degrees in either direction.
const CLLocationDirection GCLToleranceForSnappingToNorth = 7;

/// Distance threshold to stop the camera while animating.
const CLLocationDistance GCLDistanceThresholdForCameraPause = 500;

/// Reuse identifier and file name of the default point annotation image.
static NSString * const GCLDefaultStyleMarkerSymbolName = @"default_marker";

/// Reuse identifier and file name of the invisible point annotation image used
/// by annotations that are visually backed by GCLAnnotationView objects
static NSString * const GCLInvisibleStyleMarkerSymbolName = @"invisible_marker";

/// Prefix that denotes a sprite installed by GCLMapView, to avoid collisions
/// with style-defined sprites.
NSString *const GCLAnnotationSpritePrefix = @"com.mapbox.sprites.";

/// Slop area around the hit testing point, allowing for imprecise annotation selection.
const CGFloat GCLAnnotationImagePaddingForHitTest = 5;

/// Distance from the callout’s anchor point to the annotation it points to.
const CGFloat GCLAnnotationImagePaddingForCallout = 1;

const CGSize GCLAnnotationAccessibilityElementMinimumSize = CGSizeMake(10, 10);

/// An indication that the requested annotation was not found or is nonexistent.
enum { GCLAnnotationTagNotFound = UINT32_MAX };

/// Mapping from an annotation tag to metadata about that annotation, including
/// the annotation itself.
typedef std::unordered_map<GCLAnnotationTag, GCLAnnotationContext> GCLAnnotationTagContextMap;

/// Mapping from an annotation object to an annotation tag.
typedef std::map<id<GCLAnnotation>, GCLAnnotationTag> GCLAnnotationObjectTagMap;

mbgl::util::UnitBezier GCLUnitBezierForMediaTimingFunction(CAMediaTimingFunction *function)
{
    if ( ! function)
    {
...
class GCLAnnotationContext {
public:
    id <GCLAnnotation> annotation;
    /// The annotation’s image’s reuse identifier.
    NSString *imageReuseIdentifier;
    GCLAnnotationAccessibilityElement *accessibilityElement;
    GCLAnnotationView *annotationView;
    NSString *viewReuseIdentifier;
};

#pragma mark - Private -

@interface GCLMapView () <UIGestureRecognizerDelegate,
                          GLKViewDelegate,
                          CLLocationManagerDelegate,
                          GCLCalloutViewDelegate,
                          GCLCalloutViewDelegate,
                          GCLMultiPointDelegate,
                          GCLAnnotationImageDelegate>

@property (nonatomic) EAGLContext *context;
@property (nonatomic) GLKView *glView;
@property (nonatomic) UIImageView *glSnapshotView;
@property (nonatomic) NS_MUTABLE_ARRAY_OF(NSLayoutConstraint *) *scaleBarConstraints;
@property (nonatomic, readwrite) GCLScaleBar *scaleBar;
...
@property (nonatomic, readwrite) GCLStyle *style;
...
@property (nonatomic) NS_MUTABLE_DICTIONARY_OF(NSString *, GCLAnnotationImage *) *annotationImagesByIdentifier;
/// Currently shown popover representing the selected annotation.
@property (nonatomic) UIView<GCLCalloutView> *calloutViewForSelectedAnnotation;
@property (nonatomic) GCLUserLocationAnnotationView *userLocationAnnotationView;
/// Indicates how thoroughly the map view is tracking the user location.
@property (nonatomic) GCLUserTrackingState userTrackingState;
...
@property (nonatomic) GCLMapViewProxyAccessibilityElement *mapViewProxyAccessibilityElement;
@property (nonatomic) GCLAnnotationContainerView *annotationContainerView;
@property (nonatomic) GCLUserLocation *userLocation;
@property (nonatomic) NS_MUTABLE_DICTIONARY_OF(NSString *, NS_MUTABLE_ARRAY_OF(GCLAnnotationView *) *) *annotationViewReuseQueueByIdentifier;

@end

@implementation GCLMapView
{
    mbgl::Map *_mbglMap;
    MBGLView *_mbglView;
    std::unique_ptr<GCLRenderFrontend> _rendererFrontend;
    ...
    GCLAnnotationTagContextMap _annotationContextsByAnnotationTag;
    GCLAnnotationObjectTagMap _annotationTagsByAnnotation;

    /// Tag of the selected annotation. If the user location annotation is selected, this ivar is set to `GCLAnnotationTagNotFound`.
    GCLAnnotationTag _selectedAnnotationTag;
    ...
    std::vector<GCLAnnotationTag> _annotationsNearbyLastTap;
    ...
    GCLCompassDirectionFormatter *_accessibilityCompassFormatter;
    NS_ARRAY_OF(id <GCLFeature>) *_visiblePlaceFeatures;
    NS_ARRAY_OF(id <GCLFeature>) *_visibleRoadFeatures;
    NS_MUTABLE_SET_OF(GCLFeatureAccessibilityElement *) *_featureAccessibilityElements;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    {
        [self commonInit];
        self.styleURL = [self defaultStyle];
    }
    return self;
}
...
+ (void)initialize
{
    if (self == [GCLMapView self])
    {
        [GCLSDKUpdateChecker checkForUpdates];
    }
}
...
{
    NSString *styleURLString = @(_mbglMap->getStyle().getURL().c_str()).gcl_stringOrNilIfEmpty;
...
//        styleURL = [GCLStyle streetsStyleURLWithVersion:GCLStyleDefaultVersion];
        styleURL = [self defaultStyle];
    }

    styleURL = styleURL.gcl_URLByStandardizingScheme;
    self.style = nil;
...
-(NSURL*)defaultStyle{
    NSURL* styleURL = [[NSURL alloc]initWithString:@"http://ipad.geo-compass.com/styles/tdt_vec_style.json"];
    return styleURL;
}
...
- (void)commonInit
{
    _isTargetingInterfaceBuilder = NSProcessInfo.processInfo.gcl_isInterfaceBuilderDesignablesAgent;
    _opaque = NO;
...
//    self.isAccessibilityElement = YES;
    ...
    _accessibilityCompassFormatter = [[GCLCompassDirectionFormatter alloc] init];
    ...
    _rendererFrontend = std::make_unique<GCLRenderFrontend>(std::move(renderer), self, *_mbglView);
    ...
    _selectedAnnotationTag = GCLAnnotationTagNotFound;
    _annotationsNearbyLastTap = {};

    // setup logo
    //
    UIImage *logo = [GCLMapView resourceImageNamed:@"mapbox"];
    ...
    // setup scale control
    //
    _scaleBar = [[GCLScaleBar alloc] init];
    ...
    _decelerationRate = GCLMapViewDecelerationRateNormal;
    ...
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
    ...
    if ([UIApplication sharedApplication].applicationState != UIApplicationStateBackground) {
        [GCLMapboxEvents pushEvent:GCLEventTypeMapLoad withAttributes:@{}];
    }
}
...
{
    UIImage *scaleImage = [GCLMapView resourceImageNamed:@"Compass"];
...
- (void)setDelegate:(nullable id<GCLMapViewDelegate>)delegate
{
...
- (void)updateConstraintsPreiOS11 {
    ...
    // is set to YES, use its view as the parent for constraints. -[GCLMapView adjustContentInset]
...
- (void)setContentInset:(UIEdgeInsets)contentInset animated:(BOOL)animated
{
    ...
    if (self.userTrackingMode == GCLUserTrackingModeNone)
    {
...
- (void)validateDisplayLink
{
    ...
    if (isVisible && ! _displayLink)
    {
        ...
        _displayLink.frameInterval = GCLTargetFrameInterval;
...
- (void)handleCompassTapGesture:(__unused id)sender
{
    ...
    if (self.userTrackingMode == GCLUserTrackingModeFollowWithHeading ||
        self.userTrackingMode == GCLUserTrackingModeFollowWithCourse)
    {
        self.userTrackingMode = GCLUserTrackingModeFollow;
    }
}
...
- (void)touchesBegan:(__unused NS_SET_OF(UITouch *) *)touches withEvent:(__unused UIEvent *)event
{
    ...
    if (self.userTrackingState == GCLUserTrackingStateBegan)
    {
        [self setUserTrackingMode:GCLUserTrackingModeNone animated:NO];
    }
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan
{
    ...
    GCLMapCamera *oldCamera = self.camera;
    
    if (pan.state == UIGestureRecognizerStateBegan)
    {
        [self trackGestureEvent:GCLEventGesturePanStart forRecognizer:pan];

        self.userTrackingMode = GCLUserTrackingModeNone;
    ...
    else if (pan.state == UIGestureRecognizerStateChanged)
    {
        ...
        GCLMapCamera *toCamera = [self cameraByPanningWithTranslation:delta panGesture:pan];
    ...
    else if (pan.state == UIGestureRecognizerStateEnded || pan.state == UIGestureRecognizerStateCancelled)
    {
        ...
        if (self.decelerationRate == GCLMapViewDecelerationRateImmediate || sqrtf(velocity.x * velocity.x + velocity.y * velocity.y) < 100)
        {
        ...
        if (drift)
        {
            ...
            GCLMapCamera *toCamera = [self cameraByPanningWithTranslation:offset panGesture:pan];
            
            if (![self.delegate respondsToSelector:@selector(mapView:shouldChangeFromCamera:toCamera:)] ||
                [self.delegate mapView:self shouldChangeFromCamera:oldCamera toCamera:toCamera])
            {
                _mbglMap->moveBy({ offset.x, offset.y }, GCLDurationFromTimeInterval(self.decelerationRate));
            }
        }
        ...
        [GCLMapboxEvents pushEvent:GCLEventTypeMapDragEnd withAttributes:@{
            GCLEventKeyLatitude: @(panCoordinate.latitude),
            GCLEventKeyLongitude: @(panCoordinate.longitude),
            GCLEventKeyZoomLevel: @(zoom)
        }];
    }
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer *)pinch
{
    ...
    {
        [self trackGestureEvent:GCLEventGesturePinchStart forRecognizer:pinch];
    ...
    else if (pinch.state == UIGestureRecognizerStateChanged)
    {
        ...
        GCLMapCamera *toCamera = [self cameraByZoomingToZoomLevel:newZoom aroundAnchorPoint:centerPoint];
            ...
            [self.delegate mapView:self shouldChangeFromCamera:oldCamera toCamera:toCamera])
        {
            ...
            // meaningless.
            if (self.userTrackingMode == GCLUserTrackingModeNone && pinch.numberOfTouches == _previousPinchNumberOfTouches)
            {
                CLLocationCoordinate2D centerCoordinate = _previousPinchCenterCoordinate;
                _mbglMap->setLatLng(GCLLatLngFromLocationCoordinate2D(centerCoordinate),
    ...
    else if (pinch.state == UIGestureRecognizerStateEnded || pinch.state == UIGestureRecognizerStateCancelled)
    {
        ...
        GCLMapCamera *toCamera = [self cameraByZoomingToZoomLevel:zoom aroundAnchorPoint:centerPoint];
            ...
            && ![self.delegate mapView:self shouldChangeFromCamera:oldCamera toCamera:toCamera])
        {
            ...
            if (drift)
            {
                _mbglMap->setZoom(zoom, mbgl::ScreenCoordinate { centerPoint.x, centerPoint.y }, GCLDurationFromTimeInterval(duration));
            }
        }
...
- (void)handleRotateGesture:(UIRotationGestureRecognizer *)rotate
{
    ...
    {
        [self trackGestureEvent:GCLEventGestureRotateStart forRecognizer:rotate];

        self.angle = GCLRadiansFromDegrees(_mbglMap->getBearing()) * -1;
    ...
    {
        CGFloat newDegrees = GCLDegreesFromRadians(self.angle + rotate.rotation) * -1;
        ...
        GCLMapCamera *toCamera = [self cameraByRotatingToDirection:newDegrees aroundAnchorPoint:centerPoint];
    ...
    else if (rotate.state == UIGestureRecognizerStateEnded || rotate.state == UIGestureRecognizerStateCancelled)
    {
        ...
        if (decelerationRate != GCLMapViewDecelerationRateImmediate && fabs(velocity) > 3)
        {
            CGFloat radians = self.angle + rotate.rotation;
            CGFloat newRadians = radians + velocity * decelerationRate * 0.1;
            CGFloat newDegrees = GCLDegreesFromRadians(newRadians) * -1;
            ...
            if (![self.delegate respondsToSelector:@selector(mapView:shouldChangeFromCamera:toCamera:)] ||
                [self.delegate mapView:self shouldChangeFromCamera:oldCamera toCamera:toCamera])
            {
                _mbglMap->setBearing(newDegrees, mbgl::ScreenCoordinate { centerPoint.x, centerPoint.y }, GCLDurationFromTimeInterval(decelerationRate));
                
                [self notifyGestureDidEndWithDrift:YES];
                
                __weak GCLMapView *weakSelf = self;
...
- (void)handleSingleTapGesture:(UITapGestureRecognizer *)singleTap
{
    ...
    [self trackGestureEvent:GCLEventGestureSingleTap forRecognizer:singleTap];
    ...
    if (self.mapViewProxyAccessibilityElement.accessibilityElementIsFocused)
    {
        ...
        {
            if (_selectedAnnotationTag != GCLAnnotationTagNotFound) {
    ...
    id<GCLAnnotation>annotation = [self annotationForGestureRecognizer:singleTap persistingResults:YES];
    if(annotation)
    {
 ...
 */
- (nullable id <GCLAnnotation>)annotationForGestureRecognizer:(UITapGestureRecognizer*)singleTap persistingResults:(BOOL)persist
{
    ...
    for (GCLAnnotationView *view in self.annotationContainerView.annotationViews)
    {
    ...
    GCLAnnotationTag hitAnnotationTag = [self annotationTagAtPoint:tapPoint persistingResults:persist];
    if (hitAnnotationTag != GCLAnnotationTagNotFound)
    {
        if (hitAnnotationTag != _selectedAnnotationTag)
        {
            id <GCLAnnotation> annotation = [self annotationWithTag:hitAnnotationTag];
...
- (void)handleDoubleTapGesture:(UITapGestureRecognizer *)doubleTap
{
    ...
    if (doubleTap.state == UIGestureRecognizerStateEnded)
    {
        ...
        GCLMapCamera *toCamera = [self cameraByZoomingToZoomLevel:newZoom aroundAnchorPoint:gesturePoint];
        
        if (![self.delegate respondsToSelector:@selector(mapView:shouldChangeFromCamera:toCamera:)] ||
            [self.delegate mapView:self shouldChangeFromCamera:oldCamera toCamera:toCamera])
        {
            [self trackGestureEvent:GCLEventGestureDoubleTap forRecognizer:doubleTap];
            
            mbgl::ScreenCoordinate center(gesturePoint.x, gesturePoint.y);
            _mbglMap->setZoom(newZoom, center, GCLDurationFromTimeInterval(GCLAnimationDuration));
            
            __weak GCLMapView *weakSelf = self;
            
            [self animateWithDelay:GCLAnimationDuration animations:^
             {
                 [weakSelf unrotateIfNeededForGesture];
             }];
        } else {
...
- (void)handleTwoFingerTapGesture:(UITapGestureRecognizer *)twoFingerTap
{
    ...
    {
        [self trackGestureEvent:GCLEventGestureTwoFingerSingleTap forRecognizer:twoFingerTap];
    }
...
- (void)handleQuickZoomGesture:(UILongPressGestureRecognizer *)quickZoom
{
    ...
    {
        [self trackGestureEvent:GCLEventGestureQuickZoom forRecognizer:quickZoom];
...
- (void)handleTwoFingerDragGesture:(UIPanGestureRecognizer *)twoFingerDrag
{
    ...
    {
        [self trackGestureEvent:GCLEventGesturePitchStart forRecognizer:twoFingerDrag];
        [self notifyGestureDidBegin];
    }
    ...
    if (twoFingerDrag.state == UIGestureRecognizerStateBegan || twoFingerDrag.state == UIGestureRecognizerStateChanged)
    {
        ...
        GCLMapCamera *oldCamera = self.camera;
        GCLMapCamera *toCamera = [self cameraByTiltingToPitch:pitchNew];
...
- (GCLMapCamera *)cameraByPanningWithTranslation:(CGPoint)endPoint panGesture:(UIPanGestureRecognizer *)pan
{
    GCLMapCamera *panCamera = [self.camera copy];
...
- (GCLMapCamera *)cameraByZoomingToZoomLevel:(double)zoom  aroundAnchorPoint:(CGPoint)anchorPoint
{
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
...
- (GCLMapCamera *)cameraByRotatingToDirection:(CLLocationDirection)degrees aroundAnchorPoint:(CGPoint)anchorPoint
{
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
    mbgl::CameraOptions currentCameraOptions = _mbglMap->getCameraOptions(padding);
...
- (GCLMapCamera *)cameraByTiltingToPitch:(CGFloat)pitch
{
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
    mbgl::CameraOptions currentCameraOptions = _mbglMap->getCameraOptions(padding);
...
- (void)handleCalloutAccessoryTapGesture:(UITapGestureRecognizer *)tap
{
    if ([self.delegate respondsToSelector:@selector(mapView:annotation:calloutAccessoryControlTapped:)])
    {
        ...
        id <GCLAnnotation> selectedAnnotation = self.selectedAnnotation;
...
- (BOOL)calloutViewShouldHighlight:(__unused GCLCompactCalloutView *)calloutView
{
...
- (void)calloutViewTapped:(__unused GCLCompactCalloutView *)calloutView
{
    if ([self.delegate respondsToSelector:@selector(mapView:tapOnCalloutForAnnotation:)])
    {
        id <GCLAnnotation> selectedAnnotation = self.selectedAnnotation;
...
- (void)calloutViewDidAppear:(UIView<GCLCalloutView> *)calloutView
{
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    else if (gestureRecognizer == _singleTapGestureRecognizer)
    {
        ...
        {
            id<GCLAnnotation>annotation = [self annotationForGestureRecognizer:(UITapGestureRecognizer*)gestureRecognizer persistingResults:NO];
            if(!annotation) {
                return NO;
            }
        }
...
- (CLLocationDegrees)angleBetweenPoints:(CGPoint)west east:(CGPoint)east
{
    ...
    CLLocationDegrees degrees = GCLDegreesFromRadians(angle);
...
- (void)trackGestureEvent:(NSString *)gestureID forRecognizer:(UIGestureRecognizer *)recognizer
{
    ...
    [GCLMapboxEvents pushEvent:GCLEventTypeMapTap withAttributes:@{
        GCLEventKeyLatitude: @(gestureCoordinate.latitude),
        GCLEventKeyLongitude: @(gestureCoordinate.longitude),
        GCLEventKeyZoomLevel: @(zoom),
        GCLEventKeyGestureID: gestureID
    }];
}
...
- (void)showAttribution:(__unused id)sender
{
    ...
    {
        NSString *title = [info.title.string gcl_titleCasedStringWithLocale:[NSLocale currentLocale]];
        UIAlertAction *action = [UIAlertAction actionWithTitle:title
            ...
            if (url)
            {
                ...
                {
                    GCLMapCamera *camera = self.camera;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"hidden"] && object == _attributionButton)
    {
        ...
        {
            [GCLMapboxEvents ensureMetricsOptoutExists];
        }
    }
    else if ([keyPath isEqualToString:@"coordinate"] && [object conformsToProtocol:@protocol(GCLAnnotation)] && ![object isKindOfClass:[GCLMultiPoint class]])
    {
        id <GCLAnnotation> annotation = object;
        GCLAnnotationTag annotationTag = (GCLAnnotationTag)(NSUInteger)context;
        ...
        {
            const mbgl::Point<double> point = GCLPointFromLocationCoordinate2D(annotation.coordinate);

            if (annotationTag != GCLAnnotationTagNotFound) {
                GCLAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(annotationTag);
                ...
                GCLAnnotationImage *annotationImage = [self imageOfAnnotationWithTag:annotationTag];
    ...
    else if ([keyPath isEqualToString:@"coordinates"] && [object isKindOfClass:[GCLMultiPoint class]])
    {
        GCLMultiPoint *annotation = object;
        GCLAnnotationTag annotationTag = (GCLAnnotationTag)(NSUInteger)context;
        // We can get here because a subclass registered itself as an observer
...
- (GCLMapDebugMaskOptions)debugMask
{
    mbgl::MapDebugOptions options = _mbglMap->getDebug();
    GCLMapDebugMaskOptions mask = 0;
    if (options & mbgl::MapDebugOptions::TileBorders)
    {
        mask |= GCLMapDebugTileBoundariesMask;
    }
    if (options & mbgl::MapDebugOptions::ParseStatus)
    {
        mask |= GCLMapDebugTileInfoMask;
    }
    if (options & mbgl::MapDebugOptions::Timestamps)
    {
        mask |= GCLMapDebugTimestampsMask;
    }
    if (options & mbgl::MapDebugOptions::Collision)
    {
        mask |= GCLMapDebugCollisionBoxesMask;
    }
    if (options & mbgl::MapDebugOptions::Overdraw)
    {
        mask |= GCLMapDebugOverdrawVisualizationMask;
    }
    return mask;
}

- (void)setDebugMask:(GCLMapDebugMaskOptions)debugMask
{
    mbgl::MapDebugOptions options = mbgl::MapDebugOptions::NoDebug;
    if (debugMask & GCLMapDebugTileBoundariesMask)
    {
        options |= mbgl::MapDebugOptions::TileBorders;
    }
    if (debugMask & GCLMapDebugTileInfoMask)
    {
        options |= mbgl::MapDebugOptions::ParseStatus;
    }
    if (debugMask & GCLMapDebugTimestampsMask)
    {
        options |= mbgl::MapDebugOptions::Timestamps;
    }
    if (debugMask & GCLMapDebugCollisionBoxesMask)
    {
        options |= mbgl::MapDebugOptions::Collision;
    }
    if (debugMask & GCLMapDebugOverdrawVisualizationMask)
    {
...
{
    self.debugMask = debugActive ? (GCLMapDebugTileBoundariesMask |
                                    GCLMapDebugTileInfoMask |
                                    GCLMapDebugCollisionBoxesMask) : 0;
}
...
//    CLLocationDistance distance = GCLAltitudeForZoomLevel(*camera.zoom, pitch, 0, self.frame.size);
   CLLocationDistance distance = GCLAltitudeForZoomLevel(14.0, pitch, 0, self.frame.size);
    mbgl::LatLng latlng = GCLLatLngFromLocationCoordinate2D(CLLocationCoordinate2DMake(39.94, 116.3237));
    self.camera = [GCLMapCamera cameraLookingAtCenterCoordinate:GCLLocationCoordinate2DFromLatLng(latlng)
...
- (NSString *)accessibilityValue
{
    ...
    if (placeFeatures.count) {
        ...
        for (id <GCLFeature> placeFeature in placeFeatures.reverseObjectEnumerator) {
...
- (NS_ARRAY_OF(id <GCLFeature>) *)visiblePlaceFeatures
{
...
- (NS_ARRAY_OF(id <GCLFeature>) *)visibleRoadFeatures
{
...
{
    std::vector<GCLAnnotationTag> visibleAnnotations = [self annotationTagsInRect:self.bounds];
...
- (id)accessibilityElementAtIndex:(NSInteger)index
{
    ...
    if (NSLocationInRange(index, visibleAnnotationRange))
    {
        ...
        std::sort(visibleAnnotations.begin(), visibleAnnotations.end(), [&](const GCLAnnotationTag tagA, const GCLAnnotationTag tagB) {
        ...
        GCLAnnotationTag annotationTag = visibleAnnotations[annotationIndex];
        NSAssert(annotationTag != GCLAnnotationTagNotFound, @"Can’t get accessibility element for nonexistent or invisible annotation at index %li.", (long)index);
    ...
    {
        visiblePlaceFeatures = [visiblePlaceFeatures sortedArrayUsingComparator:^NSComparisonResult(id <GCLFeature> _Nonnull featureA, id <GCLFeature> _Nonnull featureB) {
        ...
        id <GCLFeature> feature = visiblePlaceFeatures[index - visiblePlaceFeatureRange.location];
    ...
    {
        visibleRoadFeatures = [visibleRoadFeatures sortedArrayUsingComparator:^NSComparisonResult(id <GCLFeature> _Nonnull featureA, id <GCLFeature> _Nonnull featureB) {
            CGPoint pointA = [self convertCoordinate:featureA.coordinate toPointToView:self];
            CGPoint pointB = [self convertCoordinate:featureB.coordinate toPointToView:self];
            CGFloat deltaA = hypot(pointA.x - centerPoint.x, pointA.y - centerPoint.y);
            CGFloat deltaB = hypot(pointB.x - centerPoint.x, pointB.y - centerPoint.y);
        ...
        id <GCLFeature> feature = visibleRoadFeatures[index - visibleRoadFeatureRange.location];
 ...
 */
- (id)accessibilityElementForAnnotationWithTag:(GCLAnnotationTag)annotationTag
{
    NSAssert(_annotationContextsByAnnotationTag.count(annotationTag), @"Missing annotation for tag %u.", annotationTag);
    GCLAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(annotationTag);
    id <GCLAnnotation> annotation = annotationContext.annotation;
    
    // Let the annotation view serve as its own accessibility element.
    GCLAnnotationView *annotationView = annotationContext.annotationView;
    ...
    {
        annotationContext.accessibilityElement = [[GCLAnnotationAccessibilityElement alloc] initWithAccessibilityContainer:self tag:annotationTag];
    }
    ...
    CGRect minimumFrame = CGRectInset({ annotationFrameCenter, CGSizeZero },
                                      -GCLAnnotationAccessibilityElementMinimumSize.width / 2,
                                      -GCLAnnotationAccessibilityElementMinimumSize.height / 2);
 ...
 */
- (id)accessibilityElementForPlaceFeature:(id <GCLFeature>)feature
{
    ...
    GCLFeatureAccessibilityElement *element = [_featureAccessibilityElements objectsPassingTest:^BOOL(GCLFeatureAccessibilityElement * _Nonnull element, BOOL * _Nonnull stop) {
    ...
    if (!element)
    {
        element = [[GCLPlaceFeatureAccessibilityElement alloc] initWithAccessibilityContainer:self feature:feature];
    }
    CGPoint center = [self convertCoordinate:feature.coordinate toPointToView:self];
    CGRect annotationFrame = CGRectInset({center, CGSizeZero}, -GCLAnnotationAccessibilityElementMinimumSize.width / 2, -GCLAnnotationAccessibilityElementMinimumSize.width / 2);
    CGRect screenRect = UIAccessibilityConvertFrameToScreenCoordinates(annotationFrame, self);
 ...
 */
- (id)accessibilityElementForRoadFeature:(id <GCLFeature>)feature
{
    if (!_featureAccessibilityElements)
    {
        _featureAccessibilityElements = [NSMutableSet set];
    ...
    if (!element)
    {
        element = [[GCLRoadFeatureAccessibilityElement alloc] initWithAccessibilityContainer:self feature:feature];
    }
    
    UIBezierPath *path;
    if ([feature isKindOfClass:[GCLPointFeature class]])
    {
        CGPoint center = [self convertCoordinate:feature.coordinate toPointToView:self];
        CGRect annotationFrame = CGRectInset({center, CGSizeZero}, -GCLAnnotationAccessibilityElementMinimumSize.width / 2, -GCLAnnotationAccessibilityElementMinimumSize.width / 2);
        CGRect screenRect = UIAccessibilityConvertFrameToScreenCoordinates(annotationFrame, self);
    ...
    else if ([feature isKindOfClass:[GCLPolylineFeature class]])
    {
        path = [self pathOfPolyline:(GCLPolyline *)feature];
    }
    else if ([feature isKindOfClass:[GCLMultiPolylineFeature class]])
    {
        path = [UIBezierPath bezierPath];
        for (GCLPolyline *polyline in [(GCLMultiPolylineFeature *)feature polylines])
        {
    ...
    if (path)
    {
        CGPathRef strokedCGPath = CGPathCreateCopyByStrokingPath(path.CGPath, NULL, GCLAnnotationAccessibilityElementMinimumSize.width, kCGLineCapButt, kCGLineJoinMiter, 0);
...
- (UIBezierPath *)pathOfPolyline:(GCLPolyline *)polyline
{
...
- (NSInteger)indexOfAccessibilityElement:(id)element
{
    ...
    std::vector<GCLAnnotationTag> visibleAnnotations = [self annotationTagsInRect:self.bounds];
    NSRange visibleAnnotationRange = NSMakeRange(NSMaxRange(userLocationAnnotationRange), visibleAnnotations.size());
    GCLAnnotationTag tag = GCLAnnotationTagNotFound;
    if ([element isKindOfClass:[GCLAnnotationView class]])
    {
        id <GCLAnnotation> annotation = [(GCLAnnotationView *)element annotation];
        tag = [self annotationTagForAnnotation:annotation];
    }
    else if ([element isKindOfClass:[GCLAnnotationAccessibilityElement class]])
    {
        tag = [(GCLAnnotationAccessibilityElement *)element tag];
    }
    
    if (tag != GCLAnnotationTagNotFound)
    {
        std::sort(visibleAnnotations.begin(), visibleAnnotations.end());
        std::sort(visibleAnnotations.begin(), visibleAnnotations.end(), [&](const GCLAnnotationTag tagA, const GCLAnnotationTag tagB) {
            CLLocationCoordinate2D coordinateA = [[self annotationWithTag:tagA] coordinate];
    ...
    NSArray *visiblePlaceFeatures = self.visiblePlaceFeatures;
    NSRange visiblePlaceFeatureRange = NSMakeRange(NSMaxRange(visibleAnnotationRange), visiblePlaceFeatures.count);
    if ([element isKindOfClass:[GCLPlaceFeatureAccessibilityElement class]])
    {
        visiblePlaceFeatures = [visiblePlaceFeatures sortedArrayUsingComparator:^NSComparisonResult(id <GCLFeature> _Nonnull featureA, id <GCLFeature> _Nonnull featureB) {
            CGPoint pointA = [self convertCoordinate:featureA.coordinate toPointToView:self];
            CGPoint pointB = [self convertCoordinate:featureB.coordinate toPointToView:self];
        ...
        id <GCLFeature> feature = [(GCLPlaceFeatureAccessibilityElement *)element feature];
        NSUInteger featureIndex = [visiblePlaceFeatures indexOfObject:feature];
        if (featureIndex == NSNotFound)
        {
            featureIndex = [visiblePlaceFeatures indexOfObjectPassingTest:^BOOL (id <GCLFeature> _Nonnull visibleFeature, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    NSArray *visibleRoadFeatures = self.visibleRoadFeatures;
    NSRange visibleRoadFeatureRange = NSMakeRange(NSMaxRange(visiblePlaceFeatureRange), visibleRoadFeatures.count);
    if ([element isKindOfClass:[GCLRoadFeatureAccessibilityElement class]])
    {
        visibleRoadFeatures = [visibleRoadFeatures sortedArrayUsingComparator:^NSComparisonResult(id <GCLFeature> _Nonnull featureA, id <GCLFeature> _Nonnull featureB) {
            CGPoint pointA = [self convertCoordinate:featureA.coordinate toPointToView:self];
            CGPoint pointB = [self convertCoordinate:featureB.coordinate toPointToView:self];
        ...
        id <GCLFeature> feature = [(GCLRoadFeatureAccessibilityElement *)element feature];
        NSUInteger featureIndex = [visibleRoadFeatures indexOfObject:feature];
        if (featureIndex == NSNotFound)
        {
            featureIndex = [visibleRoadFeatures indexOfObjectPassingTest:^BOOL (id <GCLFeature> _Nonnull visibleFeature, NSUInteger idx, BOOL * _Nonnull stop) {
                return visibleFeature.identifier && ![visibleFeature.identifier isEqual:@0] && [visibleFeature.identifier isEqual:feature.identifier];
            }];
        }
        if (featureIndex == NSNotFound)
...
- (GCLMapViewProxyAccessibilityElement *)mapViewProxyAccessibilityElement
{
    if ( ! _mapViewProxyAccessibilityElement)
    {
        _mapViewProxyAccessibilityElement = [[GCLMapViewProxyAccessibilityElement alloc] initWithAccessibilityContainer:self];
    }
...
{
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
    return GCLLocationCoordinate2DFromLatLng(_mbglMap->getLatLng(padding));
}
...
- (void)setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate zoomLevel:(double)zoomLevel direction:(CLLocationDirection)direction animated:(BOOL)animated completionHandler:(nullable void (^)(void))completion
{
    ...
    [self _setCenterCoordinate:centerCoordinate edgePadding:self.contentInset zoomLevel:zoomLevel direction:direction duration:animated ? GCLAnimationDuration : 0 animationTimingFunction:nil completionHandler:completion];
}

- (void)_setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate animated:(BOOL)animated {
    [self _setCenterCoordinate:centerCoordinate edgePadding:self.contentInset zoomLevel:self.zoomLevel direction:self.direction duration:animated ? GCLAnimationDuration : 0 animationTimingFunction:nil completionHandler:NULL];
}
...
- (void)_setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate edgePadding:(UIEdgeInsets)insets zoomLevel:(double)zoomLevel direction:(CLLocationDirection)direction duration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function completionHandler:(nullable void (^)(void))completion
{
    ...
    cameraOptions.center = GCLLatLngFromLocationCoordinate2D(centerCoordinate);
    cameraOptions.padding = GCLEdgeInsetsFromNSEdgeInsets(insets);
    cameraOptions.zoom = zoomLevel;
    if (direction >= 0)
    {
        cameraOptions.angle = GCLRadiansFromDegrees(-direction);
    }

    mbgl::AnimationOptions animationOptions;
    if (duration)
    {
        animationOptions.duration.emplace(GCLDurationFromTimeInterval(duration));
        animationOptions.easing.emplace(GCLUnitBezierForMediaTimingFunction(function));
    }
    if (completion)
    {
    ...
    GCLMapCamera *camera = [self cameraForCameraOptions:cameraOptions];
...
- (void)setZoomLevel:(double)zoomLevel animated:(BOOL)animated
{
    ...
    CGFloat duration = animated ? GCLAnimationDuration : 0;

    _mbglMap->setZoom(zoomLevel,
                      GCLEdgeInsetsFromNSEdgeInsets(self.contentInset),
                      GCLDurationFromTimeInterval(duration));
}
...
- (GCLCoordinateBounds)visibleCoordinateBounds
{
    return [self convertRect:self.bounds toCoordinateBoundsFromView:self];
}

- (void)setVisibleCoordinateBounds:(GCLCoordinateBounds)bounds
{
    [self setVisibleCoordinateBounds:bounds animated:NO];
}

- (void)setVisibleCoordinateBounds:(GCLCoordinateBounds)bounds animated:(BOOL)animated
{
    [self setVisibleCoordinateBounds:bounds edgePadding:UIEdgeInsetsZero animated:animated];
}

- (void)setVisibleCoordinateBounds:(GCLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets animated:(BOOL)animated
{
...
- (void)setVisibleCoordinateBounds:(GCLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets direction:(CLLocationDirection)direction animated:(BOOL)animated
{
    CLLocationCoordinate2D coordinates[] = {
        {bounds.ne.latitude, bounds.sw.longitude},
        bounds.sw,
...
{
    [self setVisibleCoordinates:coordinates count:count edgePadding:insets direction:direction duration:animated ? GCLAnimationDuration : 0 animationTimingFunction:nil];
}
...
{
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(insets);
    padding += GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
...
- (void)setCamera:(GCLMapCamera *)camera animated:(BOOL)animated
{
    [self setCamera:camera withDuration:animated ? GCLAnimationDuration : 0 animationTimingFunction:nil];
}

- (void)setCamera:(GCLMapCamera *)camera withDuration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function
{
    [self setCamera:camera withDuration:duration animationTimingFunction:function completionHandler:NULL];
}

- (void)setCamera:(GCLMapCamera *)camera withDuration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function completionHandler:(nullable void (^)(void))completion
{
...
- (void)setCamera:(GCLMapCamera *)camera withDuration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function edgePadding:(UIEdgeInsets)edgePadding completionHandler:(nullable void (^)(void))completion {
    mbgl::AnimationOptions animationOptions;
    if (duration > 0)
    {
        animationOptions.duration.emplace(GCLDurationFromTimeInterval(duration));
...
- (void)flyToCamera:(GCLMapCamera *)camera completionHandler:(nullable void (^)(void))completion
{
    [self flyToCamera:camera withDuration:-1 completionHandler:completion];
}

- (void)flyToCamera:(GCLMapCamera *)camera withDuration:(NSTimeInterval)duration completionHandler:(nullable void (^)(void))completion
{
    [self flyToCamera:camera withDuration:duration peakAltitude:-1 completionHandler:completion];
}

- (void)flyToCamera:(GCLMapCamera *)camera withDuration:(NSTimeInterval)duration peakAltitude:(CLLocationDistance)peakAltitude completionHandler:(nullable void (^)(void))completion
{
...
- (void)_flyToCamera:(GCLMapCamera *)camera edgePadding:(UIEdgeInsets)insets withDuration:(NSTimeInterval)duration peakAltitude:(CLLocationDistance)peakAltitude completionHandler:(nullable void (^)(void))completion
{
    mbgl::AnimationOptions animationOptions;
    if (duration >= 0)
    {
        animationOptions.duration = GCLDurationFromTimeInterval(duration);
    }
    if (peakAltitude >= 0)
    {
        ...
        animationOptions.minZoom = GCLZoomLevelForAltitude(peakAltitude, peakPitch,
...
- (GCLMapCamera *)cameraThatFitsCoordinateBounds:(GCLCoordinateBounds)bounds
{
    return [self cameraThatFitsCoordinateBounds:bounds edgePadding:UIEdgeInsetsZero];
}

- (GCLMapCamera *)cameraThatFitsCoordinateBounds:(GCLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets
{
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(insets);
    padding += GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
    mbgl::CameraOptions cameraOptions = _mbglMap->cameraForLatLngBounds(GCLLatLngBoundsFromCoordinateBounds(bounds), padding);
    return [self cameraForCameraOptions:cameraOptions];
}

- (GCLMapCamera *)cameraThatFitsShape:(GCLShape *)shape direction:(CLLocationDirection)direction edgePadding:(UIEdgeInsets)insets {
    mbgl::EdgeInsets padding = GCLEdgeInsetsFromNSEdgeInsets(insets);
    padding += GCLEdgeInsetsFromNSEdgeInsets(self.contentInset);
...
- (GCLMapCamera *)cameraForCameraOptions:(const mbgl::CameraOptions &)cameraOptions
{
    CLLocationCoordinate2D centerCoordinate = GCLLocationCoordinate2DFromLatLng(cameraOptions.center ? *cameraOptions.center : _mbglMap->getLatLng());
    double zoomLevel = cameraOptions.zoom ? *cameraOptions.zoom : self.zoomLevel;
    CLLocationDirection direction = cameraOptions.angle ? mbgl::util::wrap(-GCLDegreesFromRadians(*cameraOptions.angle), 0., 360.) : self.direction;
    CGFloat pitch = cameraOptions.pitch ? GCLDegreesFromRadians(*cameraOptions.pitch) : _mbglMap->getPitch();
    CLLocationDistance altitude = GCLAltitudeForZoomLevel(zoomLevel, pitch, centerCoordinate.latitude, self.frame.size);
    return [GCLMapCamera cameraLookingAtCenterCoordinate:centerCoordinate fromDistance:altitude pitch:pitch heading:direction];
}

/// Returns a CameraOptions object that specifies parameters for animating to
/// the given camera.
- (mbgl::CameraOptions)cameraOptionsObjectForAnimatingToCamera:(GCLMapCamera *)camera edgePadding:(UIEdgeInsets)insets
{
    mbgl::CameraOptions options;
    if (CLLocationCoordinate2DIsValid(camera.centerCoordinate))
    {
        options.center = GCLLatLngFromLocationCoordinate2D(camera.centerCoordinate);
    }
    options.padding = GCLEdgeInsetsFromNSEdgeInsets(insets);
    options.zoom = GCLZoomLevelForAltitude(camera.altitude, camera.pitch,
                                           camera.centerCoordinate.latitude,
                                           self.frame.size);
    if (camera.heading >= 0)
    {
        options.angle = GCLRadiansFromDegrees(-camera.heading);
    }
    if (camera.pitch >= 0)
    {
        options.pitch = GCLRadiansFromDegrees(camera.pitch);
    }
    return options;
}
...
{
    return GCLLocationCoordinate2DFromLatLng([self convertPoint:point toLatLngFromView:view]);
}
...
- (CGPoint)convertCoordinate:(CLLocationCoordinate2D)coordinate toPointToView:(nullable UIView *)view
{
    ...
    return [self convertLatLng:GCLLatLngFromLocationCoordinate2D(coordinate) toPointToView:view];
}
...
- (GCLCoordinateBounds)convertRect:(CGRect)rect toCoordinateBoundsFromView:(nullable UIView *)view
{
    return GCLCoordinateBoundsFromLatLngBounds([self convertRect:rect toLatLngBoundsFromView:view]);
}

- (CGRect)convertCoordinateBounds:(GCLCoordinateBounds)bounds toRectToView:(nullable UIView *)view
{
    if ( ! CLLocationCoordinate2DIsValid(bounds.sw) || ! CLLocationCoordinate2DIsValid(bounds.ne))
    {
        return CGRectNull;
    }
    return [self convertLatLngBounds:GCLLatLngBoundsFromCoordinateBounds(bounds) toRectToView:view];
}
...
- (CGRect)convertLatLngBounds:(mbgl::LatLngBounds)bounds toRectToView:(nullable UIView *)view {
    ...
    rect = GCLExtendRect(rect, [self convertLatLng:bounds.northeast() toPointToView:view]);
    return rect;
}
...
- (nullable NSString *)styleID
{
    [NSException raise:@"Method unavailable" format:
     @"%s has been replaced by -[GCLMapView styleURL].",
     __PRETTY_FUNCTION__];
    return nil;
}
...
{
    [NSException raise:@"Method unavailable" format:
     @"%s has been replaced by -[GCLMapView setStyleURL:].\n\n"
     @"If you previously set this style ID in a storyboard inspectable, select the GCLMapView in Interface Builder and delete the “styleID” entry from the User Defined Runtime Attributes section of the Identity inspector. "
...
- (nullable NS_ARRAY_OF(id <GCLAnnotation>) *)annotations
{
    ...
    std::vector<id <GCLAnnotation>> annotations;
    std::transform(_annotationContextsByAnnotationTag.begin(),
                   ...
                   ^ id <GCLAnnotation> (const std::pair<GCLAnnotationTag, GCLAnnotationContext> &pair)
    {
        return pair.second.annotation;
    });

    annotations.erase(std::remove_if(annotations.begin(), annotations.end(),
                                     [](const id <GCLAnnotation> annotation) { return annotation == nullptr; }),
                      annotations.end());

    return [NSArray arrayWithObjects:&annotations[0] count:annotations.size()];
}

- (nullable NS_ARRAY_OF(id <GCLAnnotation>) *)visibleAnnotations
{
    return [self visibleAnnotationsInRect:self.bounds];
}

- (nullable NS_ARRAY_OF(id <GCLAnnotation>) *)visibleAnnotationsInRect:(CGRect)rect
{
    if (_annotationContextsByAnnotationTag.empty())
    {
        return nil;
    ...
    std::vector<GCLAnnotationTag> annotationTags = [self annotationTagsInRect:rect];
    std::vector<GCLAnnotationTag> shapeAnnotationTags = [self shapeAnnotationTagsInRect:rect];
    ...
    if (annotationTags.size())
    {
        ...
        for (auto const& annotationTag: annotationTags)
        {
            if (!_annotationContextsByAnnotationTag.count(annotationTag) ||
                annotationTag == GCLAnnotationTagNotFound)
            {
                continue;
            }
...
- (id <GCLAnnotation>)annotationWithTag:(GCLAnnotationTag)tag
{
    if ( ! _annotationContextsByAnnotationTag.count(tag) ||
        tag == GCLAnnotationTagNotFound) {
        return nil;
    }

    GCLAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(tag);
    return annotationContext.annotation;
}

/// Returns the annotation tag assigned to the given annotation.
- (GCLAnnotationTag)annotationTagForAnnotation:(id <GCLAnnotation>)annotation
{
    if ( ! annotation || annotation == self.userLocation
        || _annotationTagsByAnnotation.count(annotation) == 0)
    {
        return GCLAnnotationTagNotFound;
    }

    return  _annotationTagsByAnnotation.at(annotation);
}

- (void)addAnnotation:(id <GCLAnnotation>)annotation
{
...
- (void)addAnnotations:(NS_ARRAY_OF(id <GCLAnnotation>) *)annotations
{
    ...
    for (id <GCLAnnotation> annotation in annotations)
    {
        NSAssert([annotation conformsToProtocol:@protocol(GCLAnnotation)], @"annotation should conform to GCLAnnotation");
        ...
        if ([annotation isKindOfClass:[GCLMultiPoint class]])
        {
            // The polyline or polygon knows how to style itself (with the map view’s help).
            GCLMultiPoint *multiPoint = (GCLMultiPoint *)annotation;
            if (!multiPoint.pointCount) {
                continue;
            }

            _isChangingAnnotationLayers = YES;
            GCLAnnotationTag annotationTag = _mbglMap->addAnnotation([multiPoint annotationObjectWithDelegate:self]);
            GCLAnnotationContext context;
        ...
        else if ( ! [annotation isKindOfClass:[GCLMultiPolyline class]]
                 && ![annotation isKindOfClass:[GCLMultiPolygon class]]
                 && ![annotation isKindOfClass:[GCLShapeCollection class]]
                 && ![annotation isKindOfClass:[GCLPointCollection class]])
        {
            GCLAnnotationView *annotationView;
            NSString *symbolName;
            ...
            if (delegateImplementsViewForAnnotation)
            {
                ...
                if (annotationView)
                {
                    ...
                    annotationView.center = GCLPointRounded([self convertCoordinate:annotation.coordinate toPointToView:self]);
                    [newAnnotationViews addObject:annotationView];

                    GCLAnnotationImage *annotationImage = self.invisibleAnnotationImage;
            ...
            if ( ! annotationView) {
                GCLAnnotationImage *annotationImage;
                ...
                if ( ! annotationImage)
                {
                    annotationImage = [self dequeueReusableAnnotationImageWithIdentifier:GCLDefaultStyleMarkerSymbolName];
                }
                if ( ! annotationImage)
                {
                ...
                if ( ! symbolName)
                {
                    symbolName = [GCLAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
            ...
            GCLAnnotationTag annotationTag = _mbglMap->addAnnotation(mbgl::SymbolAnnotation {
                GCLPointFromLocationCoordinate2D(annotation.coordinate),
                symbolName.UTF8String
            });

            GCLAnnotationContext context;
            context.annotation = annotation;
            GCLAnnotationImage *annotationImage = annotationImagesForAnnotation[annotationValue];
            ...
            if ([annotation isKindOfClass:[NSObject class]]) {
                NSAssert(![annotation isKindOfClass:[GCLMultiPoint class]], @"Point annotation should not be GCLMultiPoint.");
...
- (void)updateAnnotationContainerViewWithAnnotationViews:(NS_ARRAY_OF(GCLAnnotationView *) *)annotationViews
{
    if (annotationViews.count == 0) return;

    GCLAnnotationContainerView *newAnnotationContainerView;
    if (self.annotationContainerView)
    {
        // reload any previously added views
        newAnnotationContainerView = [GCLAnnotationContainerView annotationContainerViewWithAnnotationContainerView:self.annotationContainerView];
        [self.annotationContainerView removeFromSuperview];
    }
    else
    {
        newAnnotationContainerView = [[GCLAnnotationContainerView alloc] initWithFrame:self.bounds];
    }
...
- (GCLAnnotationImage *)defaultAnnotationImage
{
    UIImage *image = [GCLMapView resourceImageNamed:GCLDefaultStyleMarkerSymbolName];
    image = [image imageWithAlignmentRectInsets:
             UIEdgeInsetsMake(0, 0, image.size.height / 2, 0)];
    GCLAnnotationImage *annotationImage = [GCLAnnotationImage annotationImageWithImage:image
                                                                       reuseIdentifier:GCLDefaultStyleMarkerSymbolName];
    annotationImage.styleIconIdentifier = [GCLAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
    return annotationImage;
}

- (GCLAnnotationImage *)invisibleAnnotationImage
{
    GCLAnnotationImage *annotationImage = [self dequeueReusableAnnotationImageWithIdentifier:GCLInvisibleStyleMarkerSymbolName];
    ...
    if (!annotationImage)
    {
        ...
        UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
        annotationImage = [GCLAnnotationImage annotationImageWithImage:image
                                                                           reuseIdentifier:GCLInvisibleStyleMarkerSymbolName];
        annotationImage.styleIconIdentifier = [GCLAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
    }
...
- (GCLAnnotationView *)annotationViewForAnnotation:(id<GCLAnnotation>)annotation
{
    GCLAnnotationView *annotationView = [self.delegate mapView:self viewForAnnotation:annotation];
...
- (nullable GCLAnnotationView *)viewForAnnotation:(id<GCLAnnotation>)annotation
{
    if (annotation == self.userLocation)
    {
        return self.userLocationAnnotationView;
    }
    GCLAnnotationTag annotationTag = [self annotationTagForAnnotation:annotation];
    if (annotationTag == GCLAnnotationTagNotFound) {
        return nil;
    }
    GCLAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(annotationTag);
...
- (CGFloat)lineWidthForPolylineAnnotation:(GCLPolyline *)annotation
{
    ...
    {
        return [self.delegate mapView:self lineWidthForPolylineAnnotation:(GCLPolyline *)annotation];
    }
    return 3.0;
}

- (void)installAnnotationImage:(GCLAnnotationImage *)annotationImage
{
...
- (void)removeAnnotation:(id <GCLAnnotation>)annotation
{
    if ( ! annotation) return;
...
- (void)removeAnnotations:(NS_ARRAY_OF(id <GCLAnnotation>) *)annotations
{
    if ( ! annotations) return;
    ...
    for (id <GCLAnnotation> annotation in annotations)
    {
        ...
        if ([annotation isKindOfClass:[NSObject class]] && ![annotation isKindOfClass:[GCLMultiPoint class]])
        {
            [(NSObject *)annotation removeObserver:self forKeyPath:@"coordinate" context:(void *)(NSUInteger)annotationTag];
        }
        else if ([annotation isKindOfClass:[GCLMultiPoint class]])
        {
...
- (nonnull NS_ARRAY_OF(id <GCLOverlay>) *)overlays
{
    if (self.annotations == nil) { return @[]; }

    NS_MUTABLE_ARRAY_OF(id <GCLOverlay>) *mutableOverlays = [NSMutableArray array];

    [self.annotations enumerateObjectsUsingBlock:^(id<GCLAnnotation>  _Nonnull annotation, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([annotation conformsToProtocol:@protocol(GCLOverlay)])
        {
            [mutableOverlays addObject:(id<GCLOverlay>)annotation];
        }
    }];
...
- (void)addOverlay:(id <GCLOverlay>)overlay
{
    [self addOverlays:@[ overlay ]];
}

- (void)addOverlays:(NS_ARRAY_OF(id <GCLOverlay>) *)overlays
{
#if DEBUG
    for (id <GCLOverlay> overlay in overlays)
    {
        NSAssert([overlay conformsToProtocol:@protocol(GCLOverlay)], @"overlay should conform to GCLOverlay");
    }
#endif
...
- (void)removeOverlay:(id <GCLOverlay>)overlay
{
    [self removeOverlays:@[ overlay ]];
}

- (void)removeOverlays:(NS_ARRAY_OF(id <GCLOverlay>) *)overlays
{
#if DEBUG
    for (id <GCLOverlay> overlay in overlays)
    {
...
- (nullable GCLAnnotationImage *)dequeueReusableAnnotationImageWithIdentifier:(NSString *)identifier
{
    return self.annotationImagesByIdentifier[identifier];
}

- (nullable GCLAnnotationView *)dequeueReusableAnnotationViewWithIdentifier:(NSString *)identifier
{
    NSMutableArray *annotationViewReuseQueue = [self annotationViewReuseQueueForIdentifier:identifier];
    GCLAnnotationView *reusableView = annotationViewReuseQueue.firstObject;
...
/**
    ...
    @param persist True to remember the cycleable set of annotations, so that a
        ...
        asking “what if?”
 */
- (GCLAnnotationTag)annotationTagAtPoint:(CGPoint)point persistingResults:(BOOL)persist
{
    ...
    queryRect = CGRectInset(queryRect, -GCLAnnotationImagePaddingForHitTest,
                            -GCLAnnotationImagePaddingForHitTest);
    std::vector<GCLAnnotationTag> nearbyAnnotations = [self annotationTagsInRect:queryRect];
    std::vector<GCLAnnotationTag> nearbyShapeAnnotations = [self shapeAnnotationTagsInRect:queryRect];
    ...
    if (nearbyAnnotations.size())
    {
        ...
        // hit testing fails.
        auto end = std::remove_if(nearbyAnnotations.begin(), nearbyAnnotations.end(), [&](const GCLAnnotationTag annotationTag) {
            id <GCLAnnotation> annotation = [self annotationWithTag:annotationTag];
            ...
            if (annotationView)
            {
                ...
                CGPoint calloutAnchorPoint = GCLPointRounded([self convertCoordinate:annotation.coordinate toPointToView:self]);
            ...
            {
                if ([annotation isKindOfClass:[GCLShape class]])
                {
                    return false;
                }
                ...
                GCLAnnotationImage *fallbackAnnotationImage = [self dequeueReusableAnnotationImageWithIdentifier:GCLDefaultStyleMarkerSymbolName];
    ...
    GCLAnnotationTag hitAnnotationTag = GCLAnnotationTagNotFound;
    if (nearbyAnnotations.size())
    {
        ...
        if (nearbyAnnotations == _annotationsNearbyLastTap)
        {
            ...
            std::sort(nearbyAnnotations.begin(), nearbyAnnotations.end(), [&](const GCLAnnotationTag tagA, const GCLAnnotationTag tagB) {
                CLLocationCoordinate2D coordinateA = [[self annotationWithTag:tagA] coordinate];
                CLLocationCoordinate2D coordinateB = [[self annotationWithTag:tagB] coordinate];
...
- (std::vector<GCLAnnotationTag>)annotationTagsInRect:(CGRect)rect
{
...
- (std::vector<GCLAnnotationTag>)shapeAnnotationTagsInRect:(CGRect)rect
{
...
- (id <GCLAnnotation>)selectedAnnotation
{
    if (_userLocationAnnotationIsSelected)
    {
        return self.userLocation;
    ...
    GCLAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(_selectedAnnotationTag);
    return annotationContext.annotation;
}

- (void)setSelectedAnnotation:(id <GCLAnnotation>)annotation
{
...
- (NS_ARRAY_OF(id <GCLAnnotation>) *)selectedAnnotations
{
    id <GCLAnnotation> selectedAnnotation = self.selectedAnnotation;
    return (selectedAnnotation ? @[ selectedAnnotation ] : @[]);
}

- (void)setSelectedAnnotations:(NS_ARRAY_OF(id <GCLAnnotation>) *)selectedAnnotations
{
    if ( ! selectedAnnotations.count) return;

    id <GCLAnnotation> firstAnnotation = selectedAnnotations[0];

    NSAssert([firstAnnotation conformsToProtocol:@protocol(GCLAnnotation)], @"annotation should conform to GCLAnnotation");

    if ([firstAnnotation isKindOfClass:[GCLMultiPoint class]]) return;

    // Select the annotation if it’s visible.
    if (GCLCoordinateInCoordinateBounds(firstAnnotation.coordinate, self.visibleCoordinateBounds))
    {
        [self selectAnnotation:firstAnnotation animated:NO];
    }
}

- (void)selectAnnotation:(id <GCLAnnotation>)annotation animated:(BOOL)animated
{
...
- (void)selectAnnotation:(id <GCLAnnotation>)annotation animated:(BOOL)animated calloutPositioningRect:(CGRect)calloutPositioningRect
{
    if ( ! annotation) return;
    ...
    GCLAnnotationTag annotationTag = [self annotationTagForAnnotation:annotation];
    if (annotationTag == GCLAnnotationTagNotFound && annotation != self.userLocation)
    {
        [self addAnnotation:annotation];
        annotationTag = [self annotationTagForAnnotation:annotation];
        if (annotationTag == GCLAnnotationTagNotFound) return;
    }

    GCLAnnotationView *annotationView = nil;
    ...
    {
        // build the callout
        UIView <GCLCalloutView> *calloutView;
        if ([self.delegate respondsToSelector:@selector(mapView:calloutViewForAnnotation:)])
        {
            ...
            if (providedCalloutView) {
                ...
                NSAssert([providedCalloutView conformsToProtocol:@protocol(GCLCalloutView)], @"callout view must conform to GCLCalloutView");
...
- (GCLCompactCalloutView *)calloutViewForAnnotation:(id <GCLAnnotation>)annotation
{
    GCLCompactCalloutView *calloutView = [GCLCompactCalloutView platformCalloutView];
...
- (CGRect)positioningRectForAnnotation:(id <GCLAnnotation>)annotation defaultCalloutPoint:(CGPoint)calloutPoint
{
    GCLAnnotationTag annotationTag = [self annotationTagForAnnotation:annotation];
...
/// with the given tag. This rectangle is fitted to the image’s alignment rect
/// and is appropriate for positioning a popover.
- (CGRect)positioningRectForCalloutForAnnotationWithTag:(GCLAnnotationTag)annotationTag
{
    id <GCLAnnotation> annotation = [self annotationWithTag:annotationTag];
    if ( ! annotation)
    {
    ...
    if ([annotation isKindOfClass:[GCLMultiPoint class]]) {
        ...
        return CGRectMake(originPoint.x, originPoint.y, GCLAnnotationImagePaddingForHitTest, GCLAnnotationImagePaddingForHitTest);
        
    }
    UIImage *image = [self imageOfAnnotationWithTag:annotationTag].image;
    if ( ! image)
    {
        image = [self dequeueReusableAnnotationImageWithIdentifier:GCLDefaultStyleMarkerSymbolName].image;
    }
    if ( ! image)
    {
        return CGRectZero;
    ...
    return CGRectInset(positioningRect, -GCLAnnotationImagePaddingForCallout,
                       -GCLAnnotationImagePaddingForCallout);
}
...
{
    CGPoint calloutAnchorPoint = GCLPointRounded([self convertCoordinate:coordinate toPointToView:self]);
...
- (GCLAnnotationImage *)imageOfAnnotationWithTag:(GCLAnnotationTag)annotationTag
{
    if (annotationTag == GCLAnnotationTagNotFound
    ...
    NSString *symbolName = customSymbol.length ? customSymbol : GCLDefaultStyleMarkerSymbolName;

    return [self dequeueReusableAnnotationImageWithIdentifier:symbolName];
}

- (void)deselectAnnotation:(id <GCLAnnotation>)annotation animated:(BOOL)animated
{
    if ( ! annotation) return;
...
- (void)calloutViewWillAppear:(UIView <GCLCalloutView> *)calloutView
{
    if (_userLocationAnnotationIsSelected ||
...
- (void)showAnnotations:(NS_ARRAY_OF(id <GCLAnnotation>) *)annotations animated:(BOOL)animated
{
...
- (void)showAnnotations:(NS_ARRAY_OF(id <GCLAnnotation>) *)annotations edgePadding:(UIEdgeInsets)insets animated:(BOOL)animated
{
    ...
    for (id <GCLAnnotation> annotation in annotations)
    {
        if ([annotation conformsToProtocol:@protocol(GCLOverlay)])
        {
            bounds.extend(GCLLatLngBoundsFromCoordinateBounds(((id <GCLOverlay>)annotation).overlayBounds));
        }
        else
        {
            bounds.extend(GCLLatLngFromLocationCoordinate2D(annotation.coordinate));
        }
    }

    [self setVisibleCoordinateBounds:GCLCoordinateBoundsFromLatLngBounds(bounds)
                         edgePadding:insets
                            animated:animated];
}
...
- (void)annotationImageNeedsRedisplay:(GCLAnnotationImage *)annotationImage
{
    ...
    NSString *iconIdentifier = annotationImage.styleIconIdentifier;
    NSString *fallbackReuseIdentifier = GCLDefaultStyleMarkerSymbolName;
    NSString *fallbackIconIdentifier = [GCLAnnotationSpritePrefix stringByAppendingString:fallbackReuseIdentifier];
    ...
    if (annotationImage.image)
    {
        ...
        NSString *updatedIconIdentifier = [GCLAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
    ...
    else
    {
        ...
        if ( ! [self dequeueReusableAnnotationImageWithIdentifier:GCLDefaultStyleMarkerSymbolName])
        {
...
- (void)applyIconIdentifier:(NSString *)iconIdentifier toAnnotationsWithImageReuseIdentifier:(NSString *)reuseIdentifier
{
    for (auto &pair : _annotationContextsByAnnotationTag)
    {
        ...
        {
            const mbgl::Point<double> point = GCLPointFromLocationCoordinate2D(pair.second.annotation.coordinate);
...
- (void)setShowsUserLocation:(BOOL)showsUserLocation
{
    ...
    if (showsUserLocation)
    {
        ...
        self.userLocation = [[GCLUserLocation alloc] initWithMapView:self];

        GCLUserLocationAnnotationView *userLocationAnnotationView;

        if ([self.delegate respondsToSelector:@selector(mapView:viewForAnnotation:)])
        {
            userLocationAnnotationView = (GCLUserLocationAnnotationView *)[self.delegate mapView:self viewForAnnotation:self.userLocation];
            if (userLocationAnnotationView && ! [userLocationAnnotationView isKindOfClass:GCLUserLocationAnnotationView.class])
            {
                static dispatch_once_t onceToken;
                dispatch_once(&onceToken, ^{
                    NSLog(@"Ignoring user location annotation view with type %@. User location annotation view must be a kind of GCLUserLocationAnnotationView. This warning is only shown once and will become an error in a future version.", NSStringFromClass(userLocationAnnotationView.class));
                });
        ...
        self.userLocationAnnotationView = userLocationAnnotationView ?: [[GCLFaux3DUserLocationAnnotationView alloc] init];
    ...
    else
    {
        ...
        [self setUserTrackingMode:GCLUserTrackingModeNone animated:YES];
...
- (void)setUserLocationAnnotationView:(GCLUserLocationAnnotationView *)newAnnotationView
{
...
- (void)setUserTrackingMode:(GCLUserTrackingMode)mode
{
    [self setUserTrackingMode:mode animated:YES];
}

- (void)setUserTrackingMode:(GCLUserTrackingMode)mode animated:(BOOL)animated
{
    if (mode == _userTrackingMode) return;

    if ((mode == GCLUserTrackingModeFollowWithHeading || mode == GCLUserTrackingModeFollowWithCourse) &&
        ! CLLocationCoordinate2DIsValid(self.userLocation.coordinate))
    {
        mode = GCLUserTrackingModeNone;
    }

    GCLUserTrackingMode oldMode = _userTrackingMode;
    ...
    switch (_userTrackingMode)
    {
        ...
        {
            self.userTrackingState = GCLUserTrackingStatePossible;
        ...
        {
            self.userTrackingState = animated ? GCLUserTrackingStatePossible : GCLUserTrackingStateChanged;
        ...
        {
            if (oldMode == GCLUserTrackingModeNone)
            {
                self.userTrackingState = animated ? GCLUserTrackingStatePossible : GCLUserTrackingStateChanged;
            }
    ...
    if (_userTrackingMode != GCLUserTrackingModeNone)
    {
...
- (void)setUserLocationVerticalAlignment:(GCLAnnotationVerticalAlignment)alignment
{
    [self setUserLocationVerticalAlignment:alignment animated:YES];
}

- (void)setUserLocationVerticalAlignment:(GCLAnnotationVerticalAlignment)alignment animated:(BOOL)animated
{
...
- (void)setTargetCoordinate:(CLLocationCoordinate2D)targetCoordinate animated:(BOOL)animated
{
        ...
        || targetCoordinate.longitude != self.targetCoordinate.longitude)
    {
        ...
        if (self.userTrackingMode == GCLUserTrackingModeFollowWithCourse)
        {
            self.userTrackingState = GCLUserTrackingStatePossible;
...
{
    BOOL canShowPermanentHeadingIndicator = self.showsUserHeadingIndicator && self.userTrackingMode != GCLUserTrackingModeFollowWithCourse;

    if (canShowPermanentHeadingIndicator || self.userTrackingMode == GCLUserTrackingModeFollowWithHeading)
    {
        [self updateHeadingForDeviceOrientation];
...
- (void)locationManager:(__unused CLLocationManager *)manager didUpdateLocations:(NSArray *)locations animated:(BOOL)animated
{
    ...
    {
        if ( ! oldLocation || ! CLLocationCoordinate2DIsValid(oldLocation.coordinate) || self.userTrackingState != GCLUserTrackingStateBegan)
        {
    ...
    NSTimeInterval duration = GCLAnimationDuration;
    if (oldLocation && ! CGPointEqualToPoint(self.userLocationAnnotationView.center, CGPointZero))
    {
        duration = MIN([newLocation.timestamp timeIntervalSinceDate:oldLocation.timestamp], GCLUserLocationAnimationDuration);
    }
...
- (void)didUpdateLocationWithUserTrackingAnimated:(BOOL)animated
{
    ...
    else if (self.userTrackingState == GCLUserTrackingStatePossible)
    {
    ...
    else if (self.userTrackingState == GCLUserTrackingStateChanged)
    {
    ...
    [self _setCenterCoordinate:self.userLocation.location.coordinate
                   edgePadding:self.edgePaddingForFollowing
                     ...
                     direction:self.directionByFollowingWithCourse
                      duration:animated ? GCLUserLocationAnimationDuration : 0
...
- (void)didUpdateLocationSignificantlyAnimated:(BOOL)animated
{
    ...
    if (_distanceFromOldUserLocation >= GCLDistanceThresholdForCameraPause) {
        self.userTrackingState = GCLUserTrackingStateBeginSignificantTransition;
    } else {
        self.userTrackingState = GCLUserTrackingStateBegan;
    }

    GCLMapCamera *camera = self.camera;
    camera.centerCoordinate = self.userLocation.location.coordinate;
    camera.heading = self.directionByFollowingWithCourse;
    if (self.zoomLevel < GCLMinimumZoomLevelForUserTracking)
    {
        camera.altitude = GCLAltitudeForZoomLevel(GCLDefaultZoomLevelForUserTracking,
                                                  camera.pitch,
                                                  camera.centerCoordinate.latitude,
                                                  self.frame.size);
    }
    ...
    [self _flyToCamera:camera
          ...
          peakAltitude:-1
     completionHandler:^{
        GCLMapView *strongSelf = weakSelf;
        if (strongSelf.userTrackingState == GCLUserTrackingStateBegan ||
            strongSelf.userTrackingState == GCLDistanceThresholdForCameraPause)
        {
            strongSelf.userTrackingState = GCLUserTrackingStateChanged;
        }
    }];
}
...
{
    BOOL firstUpdate = self.userTrackingState == GCLUserTrackingStatePossible;
    ...
    if (self.userLocationVerticalAlignment == GCLAnnotationVerticalAlignmentCenter)
    {
...
{
    UIEdgeInsets inset = GCLUserLocationAnnotationViewInset;
...
- (CLLocationDirection)directionByFollowingWithCourse
{
    ...
    if (self.userTrackingMode == GCLUserTrackingModeFollowWithCourse)
    {
        ...
        {
            mbgl::LatLng userLatLng = GCLLatLngFromLocationCoordinate2D(self.userLocation.coordinate);
            mbgl::LatLng targetLatLng = GCLLatLngFromLocationCoordinate2D(self.targetCoordinate);
            ...
            direction = mbgl::util::wrap(GCLDegreesFromRadians(angle), 0., 360.);
        }
        else
        {
        ...
        if (direction >= 0)
        {
            if (self.userLocationVerticalAlignment == GCLAnnotationVerticalAlignmentTop)
            {
                direction += 180;
            }
        }
...
- (void)locationManager:(__unused CLLocationManager *)manager didUpdateHeading:(CLHeading *)newHeading
{
    ...
    if (self.showsUserHeadingIndicator || self.userTrackingMode == GCLUserTrackingModeFollowWithHeading)
    {
        [self updateUserLocationAnnotationView];
    }
    ...
    if (headingDirection >= 0 && self.userTrackingMode == GCLUserTrackingModeFollowWithHeading
        && self.userTrackingState != GCLUserTrackingStateBegan)
    {
...
- (NS_ARRAY_OF(id <GCLFeature>) *)visibleFeaturesAtPoint:(CGPoint)point
{
    return [self visibleFeaturesAtPoint:point inStyleLayersWithIdentifiers:nil];
}

- (NS_ARRAY_OF(id <GCLFeature>) *)visibleFeaturesAtPoint:(CGPoint)point inStyleLayersWithIdentifiers:(NS_SET_OF(NSString *) *)styleLayerIdentifiers {
    return [self visibleFeaturesAtPoint:point inStyleLayersWithIdentifiers:styleLayerIdentifiers predicate:nil];
}

- (NS_ARRAY_OF(id <GCLFeature>) *)visibleFeaturesAtPoint:(CGPoint)point inStyleLayersWithIdentifiers:(NS_SET_OF(NSString *) *)styleLayerIdentifiers predicate:(NSPredicate *)predicate
{
...
- (NS_ARRAY_OF(id <GCLFeature>) *)visibleFeaturesInRect:(CGRect)rect {
    return [self visibleFeaturesInRect:rect inStyleLayersWithIdentifiers:nil];
}

- (NS_ARRAY_OF(id <GCLFeature>) *)visibleFeaturesInRect:(CGRect)rect inStyleLayersWithIdentifiers:(NS_SET_OF(NSString *) *)styleLayerIdentifiers {
    return [self visibleFeaturesInRect:rect inStyleLayersWithIdentifiers:styleLayerIdentifiers predicate:nil];
}

- (NS_ARRAY_OF(id <GCLFeature>) *)visibleFeaturesInRect:(CGRect)rect inStyleLayersWithIdentifiers:(NS_SET_OF(NSString *) *)styleLayerIdentifiers predicate:(NSPredicate *)predicate {
...
{
    return fmaxf(_mbglMap->getMinZoom(), GCLMinimumZoom);
}
...
- (void)unrotateIfNeededForGesture
{
        ...
        && state != UIGestureRecognizerStateChanged)
    {
        ...
        // Snap to north.
        if ((self.direction < GCLToleranceForSnappingToNorth
             || self.direction > 360 - GCLToleranceForSnappingToNorth)
            && self.userTrackingMode != GCLUserTrackingModeFollowWithHeading
            && self.userTrackingMode != GCLUserTrackingModeFollowWithCourse)
        {
            [self resetNorthAnimated:YES];
        }
...
- (void)cameraWillChangeAnimated:(BOOL)animated {
    ...
    if ( ! _userLocationAnnotationIsSelected
            || self.userTrackingMode == GCLUserTrackingModeNone
            || self.userTrackingState != GCLUserTrackingStateChanged)
    {
        UIView<GCLCalloutView> *calloutView = self.calloutViewForSelectedAnnotation;
...
- (void)cameraIsChanging {
    ...
    if (!self.scaleBar.hidden) {
        [(GCLScaleBar *)self.scaleBar setMetersPerPoint:[self metersPerPointAtLatitude:self.centerCoordinate.latitude]];
    }
...
- (void)didFinishLoadingStyle {
    ...
    self.style = [[GCLStyle alloc] initWithRawStyle:&_mbglMap->getStyle() mapView:self];
...
- (void)updateAnnotationViews
{
    ...
    for (id<GCLAnnotation> annotation in visibleAnnotations)
    {
        ...
        GCLAnnotationTag annotationTag = [self annotationTagForAnnotation:annotation];
        NSAssert(annotationTag != GCLAnnotationTagNotFound, @"-visibleAnnotationsInRect: returned unrecognized annotation");
        GCLAnnotationContext &annotationContext = _annotationContextsByAnnotationTag.at(annotationTag);
        ...
        if (annotationView)
        {
            annotationView.center = GCLPointRounded([self convertCoordinate:annotationContext.annotation.coordinate toPointToView:self]);
        }
    }

    GCLCoordinateBounds coordinateBounds = [self convertRect:viewPort toCoordinateBoundsFromView:self];

    // Enqueue (and move if required) offscreen annotation views
    for (id<GCLAnnotation> annotation in offscreenAnnotations)
    {
        // Defer to the shape/polygon styling delegate methods
        if ([annotation isKindOfClass:[GCLMultiPoint class]])
        {
        ...
        if (annotationView)
        {
            ...
            // fully from view.
            if (GCLCoordinateInCoordinateBounds(coordinate, coordinateBounds))
            {
...
{
    UIView <GCLCalloutView> *calloutView = self.calloutViewForSelectedAnnotation;
    id <GCLAnnotation> annotation = calloutView.representedObject;
    ...
    {
        GCLAnnotationTag tag = [self annotationTagForAnnotation:annotation];
        GCLAnnotationView *annotationView = nil;
...
- (void)enqueueAnnotationViewForAnnotationContext:(GCLAnnotationContext &)annotationContext
{
    GCLAnnotationView *annotationView = annotationContext.annotationView;
...
{
    GCLUserLocationAnnotationView *annotationView = self.userLocationAnnotationView;
    ...
    {
        userPoint = GCLPointRounded([self convertCoordinate:self.userLocation.coordinate toPointToView:self]);
    }
    ...
    if (CGRectContainsPoint(CGRectInset(self.bounds, -GCLAnnotationUpdateViewportOutset.width,
        -GCLAnnotationUpdateViewportOutset.height), userPoint))
    {
...
- (CGPoint)userLocationAnnotationViewCenter
{
    ...
    switch (self.userLocationVerticalAlignment) {
        case GCLAnnotationVerticalAlignmentCenter:
            break;
        case GCLAnnotationVerticalAlignmentTop:
            center.y = CGRectGetMinY(contentFrame);
            break;
        case GCLAnnotationVerticalAlignmentBottom:
...
- (void)updateCompass
{
    ...
    self.compassView.transform = CGAffineTransformMakeRotation(GCLRadiansFromDegrees(plateDirection));
    ...
    {
        [UIView animateWithDuration:GCLAnimationDuration
                              delay:0
...
+ (UIImage *)resourceImageNamed:(NSString *)imageName
{
    UIImage *image = [UIImage imageNamed:imageName
                                inBundle:[NSBundle gcl_frameworkBundle]
           compatibleWithTraitCollection:nil];

    if ( ! image)
    {
        [NSException raise:@"GCLResourceNotFoundException" format:
...
- (NS_MUTABLE_ARRAY_OF(GCLAnnotationView *) *)annotationViewReuseQueueForIdentifier:(NSString *)identifier {
...
{
public:
    MBGLView(GCLMapView* nativeView_) : nativeView(nativeView_) {
    }
    ...
    void onDidFailLoadingMap(std::exception_ptr exception) override {
        ...
        try {
            ...
            code = GCLErrorCodeParseStyleFailed;
            ...
            code = GCLErrorCodeLoadStyleFailed;
        ...
        NSError *error = [NSError errorWithDomain:GCLErrorDomain code:code userInfo:userInfo];
...
private:
    __weak GCLMapView *nativeView = nullptr;
...
@implementation GCLMapView (IBAdditions)
