...
@interface GCLAnnotationView () <UIGestureRecognizerDelegate>
...
@implementation GCLAnnotationView
...
- (void)commonInitWithAnnotation:(nullable id<GCLAnnotation>)annotation reuseIdentifier:(nullable NSString *)reuseIdentifier {
...
{
    if (self.scalesWithViewingDistance == NO || self.dragState == GCLAnnotationViewDragStateDragging) return;
    ...
    if (superviewHeight > 0.0) {
        ...
        CGFloat pitchIntensity = MAX(self.mapView.camera.pitch, 0) / GCLDegreesFromRadians(mbgl::util::PITCH_MAX);
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)sender
{
    switch (sender.state) {
        case UIGestureRecognizerStateBegan:
            self.dragState = GCLAnnotationViewDragStateStarting;
            break;
        case UIGestureRecognizerStateChanged:
            self.dragState = GCLAnnotationViewDragStateDragging;
            break;
        case UIGestureRecognizerStateCancelled:
            self.dragState = GCLAnnotationViewDragStateCanceling;
            break;
        case UIGestureRecognizerStateEnded:
            self.dragState = GCLAnnotationViewDragStateEnding;
            break;
        case UIGestureRecognizerStateFailed:
            self.dragState = GCLAnnotationViewDragStateNone;
            break;
...
- (void)setDragState:(GCLAnnotationViewDragState)dragState
{
    [self setDragState:dragState animated:YES];
}

- (void)setDragState:(GCLAnnotationViewDragState)dragState animated:(BOOL)animated
{
    [self willChangeValueForKey:@"dragState"];
    _dragState = dragState;
    [self didChangeValueForKey:@"dragState"];

    if (dragState == GCLAnnotationViewDragStateStarting)
    {
        [self.mapView.calloutViewForSelectedAnnotation dismissCalloutAnimated:animated];
        [self.superview bringSubviewToFront:self];
    }
    else if (dragState == GCLAnnotationViewDragStateCanceling)
    {
        if (!self.annotation) {
    ...
    else if (dragState == GCLAnnotationViewDragStateEnding)
    {
...
{
    BOOL isDragging = self.dragState == GCLAnnotationViewDragStateDragging;
