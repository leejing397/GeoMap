...
typedef NS_ENUM(NSUInteger, GCLAnnotationViewDragState) {
    /**
     The view is not involved in a drag operation.
     */
    GCLAnnotationViewDragStateNone = 0,
    /**
     ...
     */
    GCLAnnotationViewDragStateEnding,
};

/**
 The `GCLAnnotationView` class is responsible for marking a point annotation
 with a view. Annotation views represent an annotation object, which is an
 object that corresponds to the `GCLAnnotation` protocol. When an annotation’s
 ...
 interactivity such as dragging, you can use an `GCLAnnotationImage` instead to
 conserve memory and optimize drawing performance.
 */
GCL_EXPORT
@interface GCLAnnotationView : UIView <NSSecureCoding>
...
/**
 ...
 `-[GCLMapView dequeueReusableAnnotationViewWithIdentifier:]` method.
...
/**
 ...
 with the new view and, in custom subclasses of `GCLAnnotationView`, customize the view
 ...
 with views when views are provided to the map via the `-[GCLMapViewDelegate mapView:viewForAnnotation:]`
 method.
 ...
 @return The initialized annotation view object.
 */
- (instancetype)initWithAnnotation:(nullable id<GCLAnnotation>)annotation reuseIdentifier:(nullable NSString *)reuseIdentifier;
 ...
 */
@property (nonatomic, nullable) id <GCLAnnotation> annotation;
...
/**
 ...
 viewpoint). This is also the behavior of `GCLAnnotationImage` objects. When the
...
/**
 ...
 view rotates. This is also the behavior of `GCLAnnotationImage` objects. When the
...
/**
 ...
 `GCLAnnotationView` or `GCLAnnotationImage` object, deselects any currently
 selected view.
 ...
 */
@property (nonatomic, readonly) GCLAnnotationViewDragState dragState;
 ...
 */
- (void)setDragState:(GCLAnnotationViewDragState)dragState animated:(BOOL)animated NS_REQUIRES_SUPER;
