...
static const GCLRow GCLMetricTable[] = {
...
static const GCLRow GCLImperialTable[] ={
    {.distance = 4, .numberOfBars = 2},
...
@class GCLScaleBarLabel;

@interface GCLScaleBar()
@property (nonatomic) NSArray<GCLScaleBarLabel *> *labels;
@property (nonatomic) NSArray<UIView *> *bars;
@property (nonatomic) UIView *containerView;
@property (nonatomic) GCLDistanceFormatter *formatter;
@property (nonatomic, assign) GCLRow row;
...
@interface GCLScaleBarLabel : UILabel
@end

@implementation GCLScaleBarLabel
...
- (void)commonInit {
    ...
    _formatter = [[GCLDistanceFormatter alloc] init];
}
...
- (GCLRow)preferredRow {
    ...
    GCLRow row = useMetric ? GCLMetricTable[0] : GCLImperialTable[0];
    NSUInteger count = useMetric
    ? sizeof(GCLMetricTable) / sizeof(GCLMetricTable[0])
    : sizeof(GCLImperialTable) / sizeof(GCLImperialTable[0]);
    
    for (NSUInteger i = 0; i < count; i++) {
        CLLocationDistance distance = useMetric ? GCLMetricTable[i].distance : GCLImperialTable[i].distance;
        if (distance <= maximumDistance) {
            row = useMetric ? GCLMetricTable[i] : GCLImperialTable[i];
        } else {
            break;
        }
    }
...
- (void)updateVisibility {
    ...
    ? GCLMetricTable[count-1].distance
    : GCLImperialTable[count-1].distance;
...
- (NSArray<UILabel *> *)labels {
    if (!_labels) {
        ...
        for (NSUInteger i = 0; i <= self.row.numberOfBars; i++) {
            UILabel *label = [[GCLScaleBarLabel alloc] init];
...
- (void)updateLabels {
    ...
    for (GCLScaleBarLabel *label in labels) {
...
- (void)layoutLabels {
    ...
    for (GCLScaleBarLabel *label in self.labels) {
