...
- (void)postEvents:(nonnull NS_ARRAY_OF(GCLMapboxEventAttributes *) *)events completionHandler:(nullable void (^)(NSError * _Nullable error))completionHandler {
    __block NSURLSessionDataTask *dataTask = [self.session dataTaskWithRequest:[self requestForEvents:events]
        ...
        if (httpResponse.statusCode >= 400) {
            ...
            statusError = [NSError errorWithDomain:GCLErrorDomain code:1 userInfo:userInfo];
        }
...
- (void)postEvent:(nonnull GCLMapboxEventAttributes *)event completionHandler:(nullable void (^)(NSError * _Nullable error))completionHandler {
    [self postEvents:@[event] completionHandler:completionHandler];
}

#pragma mark Utilities

- (NSURLRequest *)requestForEvents:(NS_ARRAY_OF(GCLMapboxEventAttributes *) *)events {
    NSString *path = [NSString stringWithFormat:@"%@?access_token=%@", MGLAPIClientEventsPath, [GCLAccountManager accessToken]];
    ...
    if (events.count > 2) {
        NSData *compressedData = [jsonData gcl_compressedData];
...
- (void)loadCertificates {
    NSData *certificate;
    [self loadCertificate:&certificate withResource:@"api_geocompass_com-geotrust_2016"];
    self.geoTrustCert_2016 = certificate;
    [self loadCertificate:&certificate withResource:@"api_geocompass_com-digicert_2016"];
    self.digicertCert_2016 = certificate;
    [self loadCertificate:&certificate withResource:@"api_geocompass_com-geotrust_2017"];
    self.geoTrustCert_2017 = certificate;
    [self loadCertificate:&certificate withResource:@"api_geocompass_com-digicert_2017"];
    self.digicertCert_2017 = certificate;
    [self loadCertificate:&certificate withResource:@"api_geocompass_staging"];
    self.testServerCert = certificate;
}

- (void)loadCertificate:(NSData **)certificate withResource:(NSString *)resource {
    NSBundle *frameworkBundle = [NSBundle gcl_frameworkBundle];
...
- (void)setupUserAgent {
    ...
    NSString *semanticVersion = [NSBundle gcl_frameworkInfoDictionary][@"MGLSemanticVersionString"];
    NSString *shortVersion = [NSBundle gcl_frameworkInfoDictionary][@"CFBundleShortVersionString"];
...
- (NSData *)serializedDataForEvents:(NS_ARRAY_OF(GCLMapboxEventAttributes *) *)events {
