...
#import "GCLAPIClient.h"
#import "GCLLocationManager.h"

#include <mbgl/storage/reachability.h>
#include <sys/sysctl.h>

// Event types
NSString *const GCLEventTypeAppUserTurnstile = @"appUserTurnstile";
NSString *const GCLEventTypeMapLoad = @"map.load";
NSString *const GCLEventTypeMapTap = @"map.click";
NSString *const GCLEventTypeMapDragEnd = @"map.dragend";
NSString *const GCLEventTypeLocation = @"location";
NSString *const MGLEventTypeLocalDebug = @"debug";

// Gestures
NSString *const GCLEventGestureSingleTap = @"SingleTap";
NSString *const GCLEventGestureDoubleTap = @"DoubleTap";
NSString *const GCLEventGestureTwoFingerSingleTap = @"TwoFingerTap";
NSString *const GCLEventGestureQuickZoom = @"QuickZoom";
NSString *const GCLEventGesturePanStart = @"Pan";
NSString *const GCLEventGesturePinchStart = @"Pinch";
NSString *const GCLEventGestureRotateStart = @"Rotation";
NSString *const GCLEventGesturePitchStart = @"Pitch";

// Event keys
NSString *const GCLEventKeyLatitude = @"lat";
NSString *const GCLEventKeyLongitude = @"lng";
NSString *const GCLEventKeyZoomLevel = @"zoom";
NSString *const MGLEventKeySpeed = @"speed";
NSString *const MGLEventKeyCourse = @"course";
NSString *const GCLEventKeyGestureID = @"gesture";
...
@interface GCLMapboxEvents () <GCLLocationManagerDelegate>
...
@property (nonatomic) GCLAPIClient *apiClient;
@property (nonatomic) BOOL usesTestServer;
@property (nonatomic) BOOL canEnableDebugLogging;
@property (nonatomic, getter=isPaused) BOOL paused;
@property (nonatomic) NS_MUTABLE_ARRAY_OF(GCLMapboxEventAttributes *) *eventQueue;
@property (nonatomic) dispatch_queue_t serialQueue;
@property (nonatomic) dispatch_queue_t debugLogSerialQueue;
@property (nonatomic) GCLLocationManager *locationManager;
...
@implementation GCLMapboxEvents {
    NSString *_instanceID;
    UIBackgroundTaskIdentifier _backgroundTaskIdentifier;
}

+ (void)initialize {
    if (self == [GCLMapboxEvents class]) {
...
- (instancetype) init {
    ...
    if (self) {
        ...
        _apiClient = [[GCLAPIClient alloc] init];
        ...
        _locationManager = [[GCLLocationManager alloc] init];
...
+ (nullable instancetype)sharedManager {
    ...
    static GCLMapboxEvents *_sharedManager;
...
+ (void)flush {
    [[GCLMapboxEvents sharedManager] flush];
}

- (void)flush {
    if ([GCLAccountManager accessToken] == nil) {
        return;
    }
...
- (void)pushTurnstileEvent {
    ...
    NSDictionary *turnstileEventAttributes = @{MGLEventKeyEvent: GCLEventTypeAppUserTurnstile,
...
+ (void)pushEvent:(NSString *)event withAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    [[GCLMapboxEvents sharedManager] pushEvent:event withAttributes:attributeDictionary];
}
...
- (void)pushEvent:(NSString *)event withAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    ...
    if ([event isEqualToString:GCLEventTypeMapLoad]) {
    ...
    GCLMapboxEventAttributes *fullyFormedEvent = [self fullyFormedEventForEvent:event withAttributes:attributeDictionary];
    if (fullyFormedEvent) {
...
- (GCLMapboxEventAttributes *)fullyFormedEventForEvent:(NSString *)event withAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    if ([event isEqualToString:GCLEventTypeMapLoad]) {
        return  [self mapLoadEventWithAttributes:attributeDictionary];
    } else if ([event isEqualToString:GCLEventTypeMapTap]) {
        return [self mapClickEventWithAttributes:attributeDictionary];
    } else if ([event isEqualToString:GCLEventTypeMapDragEnd]) {
        return [self mapDragEndEventWithAttributes:attributeDictionary];
    } else if ([event isEqualToString:GCLEventTypeLocation]) {
        return [self locationEventWithAttributes:attributeDictionary];
    }
    return nil;
}

- (GCLMapboxEventAttributes *)locationEventWithAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    GCLMutableMapboxEventAttributes *attributes = [NSMutableDictionary dictionary];
    attributes[MGLEventKeyEvent] = GCLEventTypeLocation;
...
- (GCLMapboxEventAttributes *)mapLoadEventWithAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    GCLMutableMapboxEventAttributes *attributes = [NSMutableDictionary dictionary];
    attributes[MGLEventKeyEvent] = GCLEventTypeMapLoad;
...
- (GCLMapboxEventAttributes *)mapClickEventWithAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    GCLMutableMapboxEventAttributes *attributes = [self interactionEvent];
    attributes[MGLEventKeyEvent] = GCLEventTypeMapTap;
    return [self eventForAttributes:attributes attributeDictionary:attributeDictionary];
}

- (GCLMapboxEventAttributes *)mapDragEndEventWithAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    GCLMutableMapboxEventAttributes *attributes = [self interactionEvent];
    attributes[MGLEventKeyEvent] = GCLEventTypeMapDragEnd;

    return [self eventForAttributes:attributes attributeDictionary:attributeDictionary];
}

- (GCLMutableMapboxEventAttributes *)interactionEvent {
    GCLMutableMapboxEventAttributes *attributes = [NSMutableDictionary dictionary];
    attributes[MGLEventKeyCreated] = [self.rfc3339DateFormatter stringFromDate:[NSDate date]];
    attributes[MGLEventKeyOrientation] = [self deviceOrientation];
    attributes[MGLEventKeyWifi] = @([[MGLReachability reachabilityForLocalWiFi] isReachableViaWiFi]);
...
- (GCLMapboxEventAttributes *)eventForAttributes:(GCLMutableMapboxEventAttributes *)attributes attributeDictionary:(GCLMapboxEventAttributes *)attributeDictionary {
...
//
- (void)postEvents:(NS_ARRAY_OF(GCLMapboxEventAttributes *) *)events {
    if (self.paused) {
        return;
    }
...
- (void)locationManager:(GCLLocationManager *)locationManager didUpdateLocations:(NSArray *)locations {
    for (CLLocation *loc in locations) {
        ...
        [GCLMapboxEvents pushEvent:GCLEventTypeLocation withAttributes:@{MGLEventKeyCreated: formattedDate,
                                                                         GCLEventKeyLatitude: @(lat),
                                                                         GCLEventKeyLongitude: @(lng),
...
#pragma mark GCLMapboxEvents Debug

- (void)pushDebugEvent:(NSString *)event withAttributes:(GCLMapboxEventAttributes *)attributeDictionary {
    ...
    GCLMutableMapboxEventAttributes *evt = [GCLMutableMapboxEventAttributes dictionaryWithDictionary:attributeDictionary];
    ...
    GCLMapboxEventAttributes *finalEvent = [NSDictionary dictionaryWithDictionary:evt];
    [self writeEventToLocalDebugLog:finalEvent];
}

- (void)writeEventToLocalDebugLog:(GCLMapboxEventAttributes *)event {
    if (![self debugLoggingEnabled]) {
        return;
    }
...
- (NSString *)stringForDebugEvent:(GCLMapboxEventAttributes *)event {
    // redact potentially sensitive location details from system console log
    if ([event[@"event"] isEqualToString:GCLEventTypeLocation]) {
        GCLMutableMapboxEventAttributes *evt = [GCLMutableMapboxEventAttributes dictionaryWithDictionary:event];
