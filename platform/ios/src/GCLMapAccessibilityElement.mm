...
@implementation GCLMapAccessibilityElement
...
@implementation GCLAnnotationAccessibilityElement

- (instancetype)initWithAccessibilityContainer:(id)container tag:(GCLAnnotationTag)tag {
...
@implementation GCLFeatureAccessibilityElement

- (instancetype)initWithAccessibilityContainer:(id)container feature:(id<GCLFeature>)feature {
    if (self = [super initWithAccessibilityContainer:container]) {
        _feature = feature;
        
        NSString *languageCode = [GCLVectorSource preferredMapboxStreetsLanguage];
...
@implementation GCLPlaceFeatureAccessibilityElement
...
@implementation GCLRoadFeatureAccessibilityElement
...
- (instancetype)initWithAccessibilityContainer:(id)container feature:(id<GCLFeature>)feature {
    if (self = [super initWithAccessibilityContainer:container feature:feature]) {
        ...
        GCLPolyline *polyline;
        if ([feature isKindOfClass:[GCLMultiPolylineFeature class]]) {
            ...
            polyline = [(GCLMultiPolylineFeature *)feature polylines].firstObject;
        }
        
        // Announce the road’s general direction.
        if ([feature isKindOfClass:[GCLPolylineFeature class]]) {
            polyline = (GCLPolylineFeature *)feature;
        }
        if (polyline) {
            NSUInteger pointCount = polyline.pointCount;
            if (pointCount) {
                CLLocationCoordinate2D *coordinates = polyline.coordinates;
                CLLocationDirection startDirection = GCLDirectionBetweenCoordinates(coordinates[pointCount - 1], coordinates[0]);
                CLLocationDirection endDirection = GCLDirectionBetweenCoordinates(coordinates[0], coordinates[pointCount - 1]);
                
                GCLCompassDirectionFormatter *formatter = [[GCLCompassDirectionFormatter alloc] init];
                formatter.unitStyle = NSFormattingUnitStyleLong;
...
@implementation GCLMapViewProxyAccessibilityElement
