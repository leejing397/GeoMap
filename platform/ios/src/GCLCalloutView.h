...
@protocol GCLCalloutViewDelegate;
 ...
 */
@protocol GCLCalloutView <NSObject>

/**
 An object conforming to the `GCLAnnotation` protocol whose details this callout
 view displays.
 */
@property (nonatomic, strong) id <GCLAnnotation> representedObject;
...
/**
 An object conforming to the `GCLCalloutViewDelegate` method that receives
 messages related to the callout view’s interactive subviews.
 */
@property (nonatomic, weak) id<GCLCalloutViewDelegate> delegate;
...
/**
 The `GCLCalloutViewDelegate` protocol defines a set of optional methods that
 you can use to receive messages from an object that conforms to the
 `GCLCalloutView` protocol. The callout view uses these methods to inform the
 delegate that the user has interacted with the the callout view.
 */
@protocol GCLCalloutViewDelegate <NSObject>
 ...
 */
- (BOOL)calloutViewShouldHighlight:(UIView<GCLCalloutView> *)calloutView;

/**
 Tells the delegate that the callout view has been tapped.
 */
- (void)calloutViewTapped:(UIView<GCLCalloutView> *)calloutView;
