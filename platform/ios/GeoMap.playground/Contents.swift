...
class MapDelegate: NSObject, MGLMapViewDelegate {
    ...
    #if swift(>=3)
    func mapView(_ mapView: GCLMapView, viewFor annotation: MGLAnnotation) -> MGLAnnotationView? {
    ...
    #else
    func mapView(mapView: GCLMapView, viewForAnnotation annotation: MGLAnnotation) -> MGLAnnotationView? {
    ...
    #else
    func mapView(mapView: GCLMapView, didSelectAnnotation annotation: MGLAnnotation) {
        let pointAnnotation = annotation as! MGLPointAnnotation
        let annotationView: PlaygroundAnnotationView  = annotationViewByAnnotation[pointAnnotation]!
    ...
    func handleTap(press: UILongPressGestureRecognizer) {
        let mapView: GCLMapView = press.view as! GCLMapView
...
let mapView = GCLMapView(frame: CGRect(x: 0, y: 0, width: width, height: height))
