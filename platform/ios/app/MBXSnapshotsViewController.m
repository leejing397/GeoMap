...
@implementation MBXSnapshotsViewController {
    // Top row
    GCLMapSnapshotter* topLeftSnapshotter;
    GCLMapSnapshotter* topCenterSnapshotter;
    GCLMapSnapshotter* topRightSnapshotter;
    
    // Bottom row
    GCLMapSnapshotter* bottomLeftSnapshotter;
    GCLMapSnapshotter* bottomCenterSnapshotter;
    GCLMapSnapshotter* bottomRightSnapshotter;
}
...
- (GCLMapSnapshotter*) startSnapshotterForImageView:(UIImageView*) imageView coordinates:(CLLocationCoordinate2D) coordinates  {
    // Create snapshot options
    GCLMapCamera* mapCamera = [[GCLMapCamera alloc] init];
    mapCamera.pitch = 20;
    mapCamera.centerCoordinate = coordinates;
    GCLMapSnapshotOptions* options = [[GCLMapSnapshotOptions alloc] initWithStyleURL:[GCLStyle satelliteStreetsStyleURL] camera:mapCamera size:CGSizeMake(imageView.frame.size.width, imageView.frame.size.height)];
    options.zoomLevel = 10;
    
    // Create and start the snapshotter
    GCLMapSnapshotter* snapshotter = [[GCLMapSnapshotter alloc] initWithOptions:options];
    [snapshotter startWithCompletionHandler: ^(GCLMapSnapshot* snapshot, NSError *error) {
        if (error) {
