...
@implementation GCLOfflinePack (MBXAdditions)
...
@implementation GCLTilePyramidOfflineRegion (MBXAdditions)

- (void)applyToMapView:(GCLMapView *)mapView {
...
- (void)viewDidLoad {
    ...
    [[GCLOfflineStorage sharedOfflineStorage] addObserver:self forKeyPath:@"packs" options:NSKeyValueObservingOptionInitial context:NULL];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(offlinePackProgressDidChange:) name:GCLOfflinePackProgressChangedNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(offlinePackDidReceiveError:) name:GCLOfflinePackErrorNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(offlinePackDidReceiveMaximumAllowedMapboxTiles:) name:GCLOfflinePackMaximumMapboxTilesReachedNotification object:nil];
}

- (void)dealloc {
    [[GCLOfflineStorage sharedOfflineStorage] removeObserver:self forKeyPath:@"packs"];
    ...
    if ([keyPath isEqualToString:@"packs"]) {
        ...
        switch (changeKind) {
            ...
            default:
                ...
                for (GCLOfflinePack *pack in [GCLOfflineStorage sharedOfflineStorage].packs) {
                    if (pack.state == GCLOfflinePackStateUnknown) {
...
- (IBAction)addCurrentRegion:(id)sender {
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = [NSString stringWithFormat:@"%@", GCLStringFromCoordinateBounds(self.mapView.visibleCoordinateBounds)];
    }];
    ...
    UIAlertAction *downloadAction = [UIAlertAction actionWithTitle:@"Download" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        GCLMapView *mapView = self.mapView;
        ...
        GCLTilePyramidOfflineRegion *region = [[GCLTilePyramidOfflineRegion alloc] initWithStyleURL:mapView.styleURL bounds:mapView.visibleCoordinateBounds fromZoomLevel:mapView.zoomLevel toZoomLevel:mapView.maximumZoomLevel];
        NSData *context = [NSKeyedArchiver archivedDataWithRootObject:@{
            MBXOfflinePackContextNameKey: name,
        }];

        [[GCLOfflineStorage sharedOfflineStorage] addPackForRegion:region withContext:context completionHandler:^(GCLOfflinePack *pack, NSError *error) {
            if (error) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [GCLOfflineStorage sharedOfflineStorage].packs.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    GCLOfflinePack *pack = [GCLOfflineStorage sharedOfflineStorage].packs[indexPath.row];

    NSString *reuseIdentifier = pack.state == GCLOfflinePackStateActive ? MBXOfflinePacksTableViewActiveCellReuseIdentifier : MBXOfflinePacksTableViewInactiveCellReuseIdentifier;
...
- (void)updateTableViewCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath forPack:(GCLOfflinePack *)pack {
    cell.textLabel.text = pack.name;
    GCLOfflinePackProgress progress = pack.progress;
    ...
    switch (pack.state) {
        ...
        case GCLOfflinePackStateInvalid:
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        GCLOfflinePack *pack = [GCLOfflineStorage sharedOfflineStorage].packs[indexPath.row];
        [[GCLOfflineStorage sharedOfflineStorage] removePack:pack withCompletionHandler:nil];
    }
}
...
- (void)offlinePackProgressDidChange:(NSNotification *)notification {
    GCLOfflinePack *pack = notification.object;
    NSAssert([pack isKindOfClass:[GCLOfflinePack class]], @"GCLOfflineStorage notification has a non-pack object.");

    NSUInteger index = [[GCLOfflineStorage sharedOfflineStorage].packs indexOfObject:pack];
    if (index == NSNotFound) {
        return;
    }
...
- (void)offlinePackDidReceiveError:(NSNotification *)notification {
    ...
    NSError *error = notification.userInfo[GCLOfflinePackUserInfoKeyError];
    NSAssert([error isKindOfClass:[NSError class]], @"GCLOfflineStorage notification has a non-error error.");
    ...
    if (error.code == GCLErrorCodeConnectionFailed) {
        NSLog(@"%@", message);
    } else {
...
- (void)offlinePackDidReceiveMaximumAllowedMapboxTiles:(NSNotification *)notification {
    ...
    uint64_t maximumCount = [notification.userInfo[GCLOfflinePackUserInfoKeyMaximumCount] unsignedLongLongValue];
