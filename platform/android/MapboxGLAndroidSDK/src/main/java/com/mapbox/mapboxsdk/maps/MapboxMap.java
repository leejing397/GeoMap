...
public final class MapboxMap {
   ...
   * wsj 2017-10-18
   */
  public static int MAP_TYPE_NORMAL = 0;
  public static int MAP_TYPE_SATELLITE = 1;
  private int mapType = -1;

  private void addTdtLayer() {
    mapType = MAP_TYPE_NORMAL;
    addTdtLayer("vec_w", "cva_w");
  }

  private void addTdtSatelliteLayer() {
    mapType = MAP_TYPE_SATELLITE;
    addTdtLayer("img_w", "ibo_w");
  }

  private void addTdtLayer(String mapName, String labelName) {
    if (this.getSource(mapName) == null) {
      RasterSource webMapSource1 = new RasterSource(
              mapName,
              new TileSet("tileset", "http://t2.tianditu.com/DataServer?T=" + mapName + "&x={x}&y={y}&l={z}"), 256);
      addSource(webMapSource1);
    }
    if (this.getSource(labelName) == null){
      RasterSource webMapSource2 = new RasterSource(
              labelName,
              new TileSet("tileset", "http://t2.tianditu.com/DataServer?T=" + labelName + "&x={x}&y={y}&l={z}"), 256);
      addSource(webMapSource2);
    }
    RasterLayer tdtLayer1 = new RasterLayer("tdt_map_layer", mapName);
    RasterLayer tdtLayer2 = new RasterLayer("tdt_name_layer", labelName);
    List<Layer> layers = getLayers();
    addLayerAt(tdtLayer1,1);
    addLayerAt(tdtLayer2,2);
  }

  /**
   * 设置底图类型
   * @param type  MAP_TYPE_NORMAL：矢量底图  MAP_TYPE_SATELLITE：卫星影像
   */
  public void setMapType(int type) {
    if (mapType == type) return;
        removeTdtLayer();
    if (type == MAP_TYPE_SATELLITE) {
      addTdtSatelliteLayer();
    } else {
      addTdtLayer();
    }
  }
   ...
   * 移除天地图图层
   */
  public void removeTdtLayer(){
    removeLayer("tdt_map_layer");
    removeLayer("tdt_name_layer");
  }
   ...
   * @param path
   */
  public void setTdtOfflinePath(String path){
