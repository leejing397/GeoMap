...
- (void)testParsing {
    ...
    NS_MUTABLE_ARRAY_OF(LAttributionInfo *) *infos = [NSMutableArray array];
    for (NSUInteger i = 0; i < sizeof(htmlStrings) / sizeof(htmlStrings[0]); i++) {
        NSArray *subinfos = [LAttributionInfo attributionInfosFromHTMLString:htmlStrings[i]
                                                                      fontSize:0
                                                                     linkColor:nil];
    ...
    NSURL *styleURL = [GCLStyle satelliteStreetsStyleURLWithVersion:99];
#if TARGET_OS_IPHONE
...
- (void)testStyle {
    ...
    CGFloat fontSize = 72;
    GCLColor *color = [GCLColor redColor];
    NS_MUTABLE_ARRAY_OF(LAttributionInfo *) *infos = [NSMutableArray array];
    for (NSUInteger i = 0; i < sizeof(htmlStrings) / sizeof(htmlStrings[0]); i++) {
        NSArray *subinfos = [LAttributionInfo attributionInfosFromHTMLString:htmlStrings[i]
                                                                      fontSize:72
    ...
    GCLColor *linkColor = [infos[0].title attribute:NSForegroundColorAttributeName atIndex:0 effectiveRange:nil];
