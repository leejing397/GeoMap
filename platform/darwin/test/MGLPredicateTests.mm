...
- (void)testFilterization {
    {
        auto actual = [NSPredicate predicateWithValue:YES].gcl_filter;
        mbgl::style::AllFilter expected;
        MGLAssertEqualFilters(actual, expected);
    }

    {
        auto actual = [NSPredicate predicateWithValue:NO].gcl_filter;
        mbgl::style::AnyFilter expected;
        MGLAssertEqualFilters(actual, expected);
    }

    {
        auto actual = [NSPredicate predicateWithFormat:@"a = 'b'"].gcl_filter;
        mbgl::style::EqualsFilter expected = { .key = "a", .value = std::string("b") };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K = 'Point'", @"$type"].gcl_filter;
        mbgl::style::TypeEqualsFilter expected = { .value = mbgl::FeatureType::Point };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K = 67086180", @"$id"].gcl_filter;
        mbgl::style::IdentifierEqualsFilter expected = { .value = UINT64_C(67086180) };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K = nil", @"$id"].gcl_filter;
        mbgl::style::NotHasIdentifierFilter expected;
        MGLAssertEqualFilters(actual, expected);
    }

    {
        auto actual = [NSPredicate predicateWithFormat:@"a = nil"].gcl_filter;
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"a BETWEEN {'b', 'z'}"].gcl_filter;
        mbgl::style::AllFilter expected = {
            .filters = {
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"a IN {'b', 'c'}"].gcl_filter;
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K IN {'LineString', 'Polygon'}", @"$type"].gcl_filter;
        mbgl::style::TypeInFilter expected = { .values = { mbgl::FeatureType::LineString, mbgl::FeatureType::Polygon } };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K IN %@", @"$type", @[@"LineString", @"Polygon"]].gcl_filter;
        mbgl::style::TypeInFilter expected = { .values = { mbgl::FeatureType::LineString, mbgl::FeatureType::Polygon } };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K IN {67086180, 3709678893, 3352016856, 4189833989}", @"$id"].gcl_filter;
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"%K IN %@", @"$id", @[@67086180, @3709678893, @3352016856, @4189833989]].gcl_filter;
        mbgl::style::IdentifierInFilter expected = { .values = { UINT64_C(67086180), UINT64_C(3709678893), UINT64_C(3352016856), UINT64_C(4189833989) } };
        MGLAssertEqualFilters(actual, expected);
    }

    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"'Mapbox' IN a"].gcl_filter, NSException, NSInvalidArgumentException);

    {
        auto actual = [NSPredicate predicateWithFormat:@"{'b', 'c'} CONTAINS a"].gcl_filter;
        mbgl::style::InFilter expected = { .key = "a", .values = { std::string("b"), std::string("c") } };
        MGLAssertEqualFilters(actual, expected);
    }

    {
        auto actual = [NSPredicate predicateWithFormat:@"%@ CONTAINS a", @[@"b", @"c"]].gcl_filter;
        mbgl::style::InFilter expected = { .key = "a", .values = { std::string("b"), std::string("c") } };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%@ CONTAINS %K", @[@"LineString", @"Polygon"], @"$type"].gcl_filter;
        mbgl::style::TypeInFilter expected = { .values = { mbgl::FeatureType::LineString, mbgl::FeatureType::Polygon } };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"{67086180, 3709678893, 3352016856, 4189833989} CONTAINS %K", @"$id"].gcl_filter;
        mbgl::style::IdentifierInFilter expected = { .values = { UINT64_C(67086180), UINT64_C(3709678893), UINT64_C(3352016856), UINT64_C(4189833989) } };
        MGLAssertEqualFilters(actual, expected);
    }
    
    {
        auto actual = [NSPredicate predicateWithFormat:@"%@ CONTAINS %K", @[@67086180, @3709678893, @3352016856, @4189833989], @"$id"].gcl_filter;
        mbgl::style::IdentifierInFilter expected = { .values = { UINT64_C(67086180), UINT64_C(3709678893), UINT64_C(3352016856), UINT64_C(4189833989) } };
        MGLAssertEqualFilters(actual, expected);
    }

    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a CONTAINS 'Mapbox'"].gcl_filter, NSException, NSInvalidArgumentException);

    {
        auto actual = [NSPredicate predicateWithFormat:@"a == 'b' AND c == 'd'"].gcl_filter;
        mbgl::style::AllFilter expected = {
            .filters = {
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"a == 'b' OR c == 'd'"].gcl_filter;
        mbgl::style::AnyFilter expected = {
            .filters = {
                mbgl::style::EqualsFilter { .key = "a", .value = std::string("b") },
                mbgl::style::EqualsFilter { .key = "c", .value = std::string("d") },
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"NOT(a == 'b' AND c == 'd')"].gcl_filter;
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"NOT(a == 'b' OR c == 'd')"].gcl_filter;
        mbgl::style::NoneFilter expected = {
            .filters = {
                mbgl::style::EqualsFilter { .key = "a", .value = std::string("b") },
                mbgl::style::EqualsFilter { .key = "c", .value = std::string("d") },
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"NOT a == nil"].gcl_filter;
        mbgl::style::HasFilter expected = { .key = "a" };
        MGLAssertEqualFilters(actual, expected);
    }
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"NOT a IN {'b', 'c'}"].gcl_filter;
    ...
    {
        auto actual = [NSPredicate predicateWithFormat:@"NOT {'b', 'c'} CONTAINS a"].gcl_filter;
        mbgl::style::NotInFilter expected = { .key = "a", .values = { std::string("b"), std::string("c") } };
        MGLAssertEqualFilters(actual, expected);
    }

    {
        auto actual = [NSPredicate predicateWithFormat:@"NOT %@ CONTAINS a", @[@"b", @"c"]].gcl_filter;
        mbgl::style::NotInFilter expected = { .key = "a", .values = { std::string("b"), std::string("c") } };
        MGLAssertEqualFilters(actual, expected);
    }

    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a BEGINSWITH 'L'"].gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a ENDSWITH 'itude'"].gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a LIKE 'glob?trotter'"].gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a MATCHES 'i\\w{18}n'"].gcl_filter, NSException, NSInvalidArgumentException);
    NSPredicate *selectorPredicate = [NSPredicate predicateWithFormat:@"(SELF isKindOfClass: %@)", [MGLPolyline class]];
    XCTAssertThrowsSpecificNamed(selectorPredicate.gcl_filter, NSException, NSInvalidArgumentException);
    ...
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithBlock:^BOOL(id _Nullable evaluatedObject, NSDictionary<NSString *, id> * _Nullable bindings) {
        ...
        return NO;
    }].gcl_filter, NSException, NSInvalidArgumentException);
}
...
- (void)testSymmetry {
    ...
    [self testSymmetryWithFormat:@"a != 1" reverseFormat:@"1 != a" mustRoundTrip:YES];
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a = b"].gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"1 = 1"].gcl_filter, NSException, NSInvalidArgumentException);
    ...
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"$id == 670861802"].gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a = $id"].gcl_filter, NSException, NSInvalidArgumentException);
    ...
    [self testSymmetryWithPredicate:[NSPredicate predicateWithFormat:@"a BETWEEN %@", @[@1, @2]]
                   reversePredicate:[NSPredicate predicateWithFormat:@"1 <= a && 2 >= a"]
                      mustRoundTrip:YES];
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"{1, 2} BETWEEN a"].gcl_filter, NSException, NSInvalidArgumentException);
    NSPredicate *betweenSetPredicate = [NSPredicate predicateWithFormat:@"a BETWEEN %@", [NSSet setWithObjects:@1, @2, nil]];
    XCTAssertThrowsSpecificNamed(betweenSetPredicate.gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a BETWEEN {1}"].gcl_filter, NSException, NSInvalidArgumentException);
    XCTAssertThrowsSpecificNamed([NSPredicate predicateWithFormat:@"a BETWEEN {1, 2, 3}"].gcl_filter, NSException, NSInvalidArgumentException);
...
- (void)testSymmetryWithPredicate:(NSPredicate *)forwardPredicate reversePredicate:(NSPredicate *)reversePredicate mustRoundTrip:(BOOL)mustRoundTrip {
    auto forwardFilter = forwardPredicate.gcl_filter;
    ...
    if (reversePredicate) {
        auto reverseFilter = reversePredicate.gcl_filter;
