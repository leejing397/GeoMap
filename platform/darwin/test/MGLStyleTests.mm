...
@interface MGLStyleTests : XCTestCase <GCLMapViewDelegate>

@property (nonatomic) GCLMapView *mapView;
@property (nonatomic) GCLStyle *style;
...
- (void)setUp {
    ...
    self.mapView = [[GCLMapView alloc] initWithFrame:CGRectMake(0, 0, 100, 100) styleURL:styleURL];
...
- (void)testUnversionedStyleURLs {
    ...
    XCTAssertEqualObjects([GCLStyle streetsStyleURL].absoluteString, @(mbgl::util::default_styles::streets.url));
    XCTAssertEqualObjects([GCLStyle outdoorsStyleURL].absoluteString, @(mbgl::util::default_styles::outdoors.url));
    XCTAssertEqualObjects([GCLStyle lightStyleURL].absoluteString, @(mbgl::util::default_styles::light.url));
    XCTAssertEqualObjects([GCLStyle darkStyleURL].absoluteString, @(mbgl::util::default_styles::dark.url));
    XCTAssertEqualObjects([GCLStyle satelliteStyleURL].absoluteString, @(mbgl::util::default_styles::satellite.url));
    XCTAssertEqualObjects([GCLStyle satelliteStreetsStyleURL].absoluteString, @(mbgl::util::default_styles::satelliteStreets.url));
    
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    XCTAssertEqualObjects([GCLStyle emeraldStyleURL].absoluteString, @"mapbox://styles/mapbox/emerald-v8");
    XCTAssertEqualObjects([GCLStyle hybridStyleURL].absoluteString, @"mapbox://styles/mapbox/satellite-hybrid-v8");
#pragma clang diagnostic pop
}

- (void)testVersionedStyleURLs {
    // Test that all the default styles have publicly-declared GCLStyle class
    // methods and that the URLs all have the right values.
    XCTAssertEqualObjects([GCLStyle streetsStyleURLWithVersion:mbgl::util::default_styles::streets.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::streets.url));
    XCTAssertEqualObjects([GCLStyle streetsStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/streets-v99");
    XCTAssertEqualObjects([GCLStyle outdoorsStyleURLWithVersion:mbgl::util::default_styles::outdoors.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::outdoors.url));
    XCTAssertEqualObjects([GCLStyle outdoorsStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/outdoors-v99");
    XCTAssertEqualObjects([GCLStyle lightStyleURLWithVersion:mbgl::util::default_styles::light.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::light.url));
    XCTAssertEqualObjects([GCLStyle lightStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/light-v99");
    XCTAssertEqualObjects([GCLStyle darkStyleURLWithVersion:mbgl::util::default_styles::dark.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::dark.url));
    XCTAssertEqualObjects([GCLStyle darkStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/dark-v99");
    XCTAssertEqualObjects([GCLStyle satelliteStyleURLWithVersion:mbgl::util::default_styles::satellite.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::satellite.url));
    XCTAssertEqualObjects([GCLStyle satelliteStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/satellite-v99");
    XCTAssertEqualObjects([GCLStyle satelliteStreetsStyleURLWithVersion:mbgl::util::default_styles::satelliteStreets.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::satelliteStreets.url));
    XCTAssertEqualObjects([GCLStyle satelliteStreetsStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/satellite-streets-v99");
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    XCTAssertEqualObjects([GCLStyle trafficDayStyleURLWithVersion:mbgl::util::default_styles::trafficDay.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::trafficDay.url));
    XCTAssertEqualObjects([GCLStyle trafficDayStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/traffic-day-v99");
    XCTAssertEqualObjects([GCLStyle trafficNightStyleURLWithVersion:mbgl::util::default_styles::trafficNight.currentVersion].absoluteString,
                          @(mbgl::util::default_styles::trafficNight.url));
    XCTAssertEqualObjects([GCLStyle trafficNightStyleURLWithVersion:99].absoluteString,
                          @"mapbox://styles/mapbox/traffic-night-v99");
#pragma clang diagnostic pop

    static_assert(8 == mbgl::util::default_styles::numOrderedStyles,
                  "GCLStyleTests isn’t testing all the styles in mbgl::util::default_styles.");
}
...
- (void)testStyleURLDeclarations {
    ...
    for (NSUInteger i = 0; i < numMethods; i++) {
        ...
        if ([name hasSuffix:@"StyleURL"]) {
            ...
            XCTAssertEqual([name rangeOfString:@"URL"].location, NSNotFound, @"GCLStyle style URL method %@ is malformed.", name);
        }
    }
...
- (void)testSources {
    NSSet<GCLSource *> *initialSources = self.style.sources;
...
- (void)testAddingLayersTwice {
    ...
    GCLLineStyleLayer *lineLayer = [[GCLLineStyleLayer alloc] initWithIdentifier:@"lineLayer" source:source];
...
- (void)testRemovingLayerBeforeAddingSameLayer {
    ...
    GCLLineStyleLayer *lineLayer = [[GCLLineStyleLayer alloc] initWithIdentifier:@"line-layer" source:source];
...
- (void)testAddingLayerOfTypeABeforeRemovingLayerOfTypeBWithSameIdentifier {
    ...
    GCLLineStyleLayer *lineLayer = [[GCLLineStyleLayer alloc] initWithIdentifier:fillLayer.identifier source:source];
    [self.style removeLayer:lineLayer];
...
- (NSString *)stringWithContentsOfStyleHeader {
    NSURL *styleHeaderURL = [[[NSBundle mgl_frameworkBundle].bundleURL
                             ...
                             URLByAppendingPathComponent:@"GCLStyle.h"];
    ...
    XCTAssertNil(styleHeaderError, @"Error getting contents of GCLStyle.h.");
    return styleHeader;
}
...
#if TARGET_OS_IPHONE
    GCLImage *image = [GCLImage imageNamed:imageName
                                  inBundle:[NSBundle bundleForClass:[self class]]
             compatibleWithTraitCollection:nil];
#else
    GCLImage *image = [[NSBundle bundleForClass:[self class]] imageForResource:imageName];
#endif
    XCTAssertNotNil(image);

    [self.style setImage:image forName:imageName];
    GCLImage *styleImage = [self.style imageForName:imageName];
