...
typealias GCLColor = UIColor
#elseif os(macOS)
typealias GCLColor = NSColor
#endif
...
extension MGLStyleValueTests {
    ...
    func assertColorsEqualWithAccuracy(_ actual: GCLColor, _ expected: GCLColor, accuracy: Float = 1/255) {
    ...
    func assertColorValuesEqual(_ actual: MGLStyleValue<GCLColor>, _ expected: MGLStyleValue<GCLColor>) {
        guard type(of: actual) == type(of: expected) else {
            XCTFail("Expected \(type(of: expected)), but found \(type(of: actual)) instead.")
            return
        }
        
        if let actualConstant = actual as? MGLConstantStyleValue<GCLColor> {
            assertColorsEqualWithAccuracy(actualConstant.rawValue, (expected as! MGLConstantStyleValue<GCLColor>).rawValue)
        } else if let actualFunction = actual as? MGLStyleFunction<GCLColor>,
            let expectedFunction = expected as? MGLStyleFunction<GCLColor> {
            ...
            if let actualFunction = actualFunction as? MGLSourceStyleFunction<GCLColor>,
                let expectedFunction = expectedFunction as? MGLSourceStyleFunction<GCLColor> {
                XCTAssertEqual(actualFunction.defaultValue, expectedFunction.defaultValue)
            } else if let actualFunction = actualFunction as? MGLCompositeStyleFunction<GCLColor>,
                let expectedFunction = expectedFunction as? MGLCompositeStyleFunction<GCLColor> {
                XCTAssertEqual(actualFunction.defaultValue, expectedFunction.defaultValue)
            }
            ...
            func assertStopEqual (_ actualValue: Any?, _ expectedValue: Any?) {
                ...
                if let actualValue = actualValue as? MGLConstantStyleValue<GCLColor>,
                    let expectedValue = expectedValue as? MGLConstantStyleValue<GCLColor> {
            ...
            XCTFail("MGLStyleValue<GCLColor> must be either a constant or a style function.")
        }
    }
    ...
    func testFunctionsWithDataDrivenProperties() {
        ...
        let redGreenStops : [Float:MGLStyleValue<GCLColor>] = [
            0: MGLStyleValue<GCLColor>(rawValue: .red),
            10: MGLStyleValue<GCLColor>(rawValue: .red),
            15: MGLStyleValue<GCLColor>(rawValue: .green)
        ]
        let expectedCircleColorValue = MGLStyleValue<GCLColor>(
        ...
        assertColorValuesEqual(circleStyleLayer.circleColor as! MGLStyleFunction<GCLColor>, expectedCircleColorValue as! MGLStyleFunction<GCLColor>)
        
        // data-driven, source function with categorical color stop values with string attribute keys
        let redOnlyStops = [
            "red": MGLStyleValue<GCLColor>(rawValue: .red)
        ]
        let expectedRedCategoricalValue = MGLStyleValue<GCLColor>(
            interpolationMode: .categorical,
            sourceStops: redOnlyStops,
            attributeName: "red",
            options: [.defaultValue: MGLStyleValue<GCLColor>(rawValue: .cyan)]
        )
        ...
        let greenOrangeStops : [Float:MGLStyleValue<GCLColor>] = [
            0: MGLStyleValue<GCLColor>(rawValue: .green),
            100: MGLStyleValue<GCLColor>(rawValue: .orange)
        ]
        let expectedGreenOrangeCategoricalValue = MGLStyleValue<GCLColor>(
            interpolationMode: .categorical,
            sourceStops: greenOrangeStops,
            attributeName: "temp",
            options: [.defaultValue: MGLStyleValue<GCLColor>(rawValue: .red)]
        )
        ...
        let expectedRedGreenSourceExponentialValue = MGLStyleValue<GCLColor>(
            interpolationMode: .exponential,
        ...
        let expectedSourceIdentityValue = MGLStyleValue<GCLColor>(
            interpolationMode: .identity,
            sourceStops: nil,
            attributeName: "size",
            options: [.defaultValue: MGLStyleValue<GCLColor>(rawValue: .green)]
        )
