...
- (void)testGeometryConversion {
    ...
    NS_ARRAY_OF(GCLShape <GCLFeature> *) *shapes = MGLFeaturesFromMBGLFeatures(features);
    XCTAssertEqual(shapes.count, 3, @"All features should be converted into shapes");

    GCLPointFeature *pointShape = (GCLPointFeature *)shapes[0];
    XCTAssertTrue([pointShape isKindOfClass:[GCLPointFeature class]]);
    ...
    GCLPolylineFeature *polylineShape = (GCLPolylineFeature *)shapes[1];
    XCTAssertTrue([polylineShape isKindOfClass:[GCLPolylineFeature class]]);
    ...
    GCLPolygonFeature *polygonShape = (GCLPolygonFeature *)shapes[2];
    XCTAssertTrue([polygonShape isKindOfClass:[GCLPolygonFeature class]]);
...
- (void)testPropertyConversion {
    ...
    GCLShape <GCLFeature> *shape = shapes.firstObject;
    XCTAssertTrue([shape conformsToProtocol:@protocol(GCLFeature)]);
    XCTAssertTrue([shape isKindOfClass:[GCLShape class]]);
...
- (void)testPolylineFeatureGeoJSONDictionary {
    ...
    GCLPolylineFeature *polyLineFeature = [GCLPolylineFeature polylineWithCoordinates:coords count:2];
...
- (void)testPolygonFeatureGeoJSONDictionary {
    ...
    GCLPolygonFeature *polygonFeature = [GCLPolygonFeature polygonWithCoordinates:coords count:3];
...
- (void)testMultiPolylineFeatureGeoJSONDictionary {
    ...
    GCLMultiPolylineFeature *multiPolylineFeature = [MGLMultiPolylineFeature multiPolylineWithPolylines:@[polyLine1, polyLine2]];
...
- (void)testMultiPolygonFeatureGeoJSONDictionary {
    ...
    GCLMultiPolygonFeature *multiPolylineFeature = [MGLMultiPolygonFeature multiPolygonWithPolygons:@[polygon1, polygon2]];
...
- (void)testPointCollectionFeatureGeoJSONDictionary {
    ...
    CLLocationCoordinate2D coord3 = { 0, 0 };
    CLLocationCoordinate2D coords[] = { coord1, coord2, coord3 };
    GCLPointCollectionFeature *pointCollectionFeature = [GCLPointCollectionFeature pointCollectionWithCoordinates:coords count:3];
...
- (void)testShapeCollectionFeatureGeoJSONDictionary {
    ...
    GCLShapeCollectionFeature *shapeCollectionFeature = [MGLShapeCollectionFeature shapeCollectionWithShapes:@[pointFeature,
                                                                                                             polyline]];
    // A GeoJSON feature
    ...
    shapeCollectionFeature = [GCLShapeCollectionFeature shapeCollectionWithShapes:@[]];
