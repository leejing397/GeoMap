...
// object identity. We don't store a GCLSource pointer directly because that doesn't
// interoperate with ARC. The inner pointer is weak in order to avoid a reference cycle for
// "pending" GCLSources, which have a strong owning pointer to the mbgl::style::Source.
struct SourceWrapper {
    __weak GCLSource *source;
};

@class GCLMapView;

@interface GCLSource (Private)

/**
 Initializes and returns a source with a raw pointer to the backing store,
 associated with a style.
 */
- (instancetype)initWithRawSource:(mbgl::style::Source *)rawSource mapView:(nullable GCLMapView *)mapView;
...
/**
 ...
 can still be changed but the changes will not be visible until the `GCLSource`
 is added back to the map via `-[GCLStyle addSource:]` and styled with a
 `GCLLayer`.
 */
...
/**
 ...
 to the `GCLSource` instance and the unique_ptr reference is valid again. It is
