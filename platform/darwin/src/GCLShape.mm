...
+ (nullable GCLShape *)shapeWithData:(NSData *)data encoding:(NSStringEncoding)encoding error:(NSError * _Nullable *)outError {
    ...
    if (!string) {
        if (outError) {
            *outError = [NSError errorWithDomain:GCLErrorDomain code:GCLErrorCodeUnknown userInfo:nil];
        }
        return nil;
    }
    ...
    try {
        ...
        return GCLShapeFromGeoJSON(geojson);
    } catch (std::runtime_error &err) {
        if (outError) {
            *outError = [NSError errorWithDomain:GCLErrorDomain code:GCLErrorCodeUnknown userInfo:@{
...
- (mbgl::Geometry<double>)geometryObject {
    [NSException raise:@"GCLAbstractClassException"
                format:@"GCLShape is an abstract class"];
...
- (BOOL)isEqual:(id)other
{
    ...
    id <GCLAnnotation> annotation = other;
...
{
    [[NSException exceptionWithName:@"GCLAbstractClassException"
                             reason:@"GCLShape is an abstract class"
                           userInfo:nil] raise];
