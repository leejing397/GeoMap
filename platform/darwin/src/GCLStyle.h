...
/**
 ...
 ascertain the style used by `GCLMapView` and `GCLTilePyramidOfflineRegion` when
 ...
 @warning The value of this constant may change in a future release of the SDK.
    ...
    version.
 */
static GCL_EXPORT const NSInteger GCLStyleDefaultVersion = 10;

/**
 The proxy object for the current map style.

 GCLStyle provides a set of convenience methods for changing Mapbox
 default styles using `-[GCLMapView styleURL]`.
 ...
 via `-[GCLMapView style]` by updating the style's data sources or layers.

 @note Wait until the map style has finished loading before modifying a map's
    style via any of the `GCLStyle` instance methods below. You can use the
    `-[GCLMapViewDelegate mapView:didFinishLoadingStyle:]` or
    `-[GCLMapViewDelegate mapViewDidFinishLoadingMap:]` methods as indicators
    that it's safe to modify the map's style.
 */
GCL_EXPORT
@interface GCLStyle : NSObject
...
/**
 ...
 `GCLMapView` and `GCLTilePyramidOfflineRegion` use Mapbox Streets when no style
 ...
 */
@property (nonatomic, strong) NS_SET_OF(__kindof GCLSource *) *sources;

/**
 Values describing animated transitions to changes on a style's individual
 paint properties.
 */
@property (nonatomic) GCLTransition transition;
...
/**
 ...
 @return An instance of a concrete subclass of `GCLSource` associated with the
    given identifier, or `nil` if the current style contains no such source.
 */
- (nullable GCLSource *)sourceWithIdentifier:(NSString *)identifier;

/**
 Adds a new source to the current style.

 @note Adding the same source instance more than once will result in a
    `GCLRedundantSourceException`. Reusing the same source identifier, even with
    different source instances, will result in a
    `GCLRedundantSourceIdentifierException`.

 @note Sources should be added in
    `-[GCLMapViewDelegate mapView:didFinishLoadingStyle:]` or
    `-[GCLMapViewDelegate mapViewDidFinishLoadingMap:]` to ensure that the map
 ...
 */
@property (nonatomic, strong) NS_ARRAY_OF(__kindof GCLStyleLayer *) *layers;
...
/**
 ...
 @return An instance of a concrete subclass of `GCLStyleLayer` associated with
    the given identifier, or `nil` if the current style contains no such style
    layer.
 */
- (nullable GCLStyleLayer *)layerWithIdentifier:(NSString *)identifier;

/**
 Adds a new layer on top of existing layers.

 @note Adding the same layer instance more than once will result in a
    `GCLRedundantLayerException`. Reusing the same layer identifer, even with
 ...
 @param layer The layer object to add to the map view. This object must be an
    instance of a concrete subclass of `GCLStyleLayer`.
 */
...
/**
 ...
 must conform to the `GCLStyleLayer` protocol.
 */
...
/**
 ...
 @note Names and their associated images are not guaranteed to exist across
    ...
    convenience method like `+[GCLStyle outdoorsStyleURLWithVersion:]`,
    `GCLMapView`’s “Style URL” inspectable in Interface Builder, or a manually
 ...
 */
- (nullable GCLImage *)imageForName:(NSString *)name;
...
/**
 ...
 set the `iconImageName` property of an `GCLSymbolStyleLayer` object to that
 name.
 ...
 */
@property (nonatomic, strong) GCLLight *light;
