...
@implementation LAttributionInfo

+ (NS_ARRAY_OF(LAttributionInfo *) *)attributionInfosFromHTMLString:(nullable NSString *)htmlString fontSize:(CGFloat)fontSize linkColor:(nullable MGLColor *)linkColor {
    if (!htmlString) {
        return @[];
    }
    ...
    ^(id _Nullable value, NSRange range, BOOL * _Nonnull stop) {
        ...
        LAttributionInfo *info = [[LAttributionInfo alloc] initWithTitle:unlinkedTitle URL:value];
...
+ (NSAttributedString *)attributedStringForAttributionInfos:(NS_ARRAY_OF(LAttributionInfo *) *)attributionInfos {
    NSMutableArray *titles = [NSMutableArray arrayWithCapacity:attributionInfos.count];
    for (LAttributionInfo *info in attributionInfos) {
 ...
 */
- (NSComparisonResult)subsetCompare:(LAttributionInfo *)otherInfo {
...
- (void)growArrayByAddingAttributionInfo:(LAttributionInfo *)info {
    __block BOOL didInsertInfo = NO;
    __block BOOL shouldAddInfo = YES;
    [self enumerateObjectsUsingBlock:^(LAttributionInfo * _Nonnull existingInfo, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)growArrayByAddingAttributionInfosFromArray:(NS_ARRAY_OF(LAttributionInfo *) *)infos {
    for (LAttributionInfo *info in infos) {
