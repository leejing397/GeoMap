...
/**
 An `GCLPointCollection` object represents a shape consisting of one or more
 ...
 You can add point collections to the map by adding them to an `GCLShapeSource`
 object. Configure the appearance of an `GCLShapeSource`’s or
 `GCLVectorSource`’s point collections collectively using an
 `GCLCircleStyleLayer` or `GCLSymbolStyleLayer` object.

 You cannot add an `GCLPointCollection` object directly to a map view as an
 annotation. However, you can create individual `GCLPointAnnotation` objects
 from the `coordinates` array and add those annotation objects to the map view
 using the `-[GCLMapView addAnnotations:]` method.

 A point collection is known as a
 <a href="https://tools.ietf.org/html/rfc7946#section-3.1.3">MultiPoint</a>
 geometry in GeoJSON. Do not confuse `GCLPointCollection` with `GCLMultiPoint`,
 the abstract superclass of `GCLPolyline` and `GCLPolygon`.
 */
GCL_EXPORT
@interface GCLPointCollection : GCLShape <GCLOverlay>

/**
 Creates and returns a `GCLPointCollection` object from the specified set of
 coordinates.
