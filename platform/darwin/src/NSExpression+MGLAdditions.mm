...
@implementation NSExpression (GCLAdditions)

- (std::vector<mbgl::Value>)gcl_aggregateMBGLValue {
    if ([self.constantValue isKindOfClass:[NSArray class]] || [self.constantValue isKindOfClass:[NSSet class]]) {
        ...
        for (id value in self.constantValue) {
            ...
            convertedValues.push_back(expression.gcl_constantMBGLValue);
        }
...
- (mbgl::Value)gcl_constantMBGLValue {
    ...
    } else if ([value isKindOfClass:[GCLColor class]]) {
        auto hexString = [(GCLColor *)value mgl_color].stringify();
        return { hexString };
...
- (std::vector<mbgl::FeatureType>)gcl_aggregateFeatureType {
    if ([self.constantValue isKindOfClass:[NSArray class]] || [self.constantValue isKindOfClass:[NSSet class]]) {
        ...
        for (id value in self.constantValue) {
            ...
            convertedValues.push_back(expression.gcl_featureType);
        }
        return convertedValues;
    }
    [NSException raise:NSInvalidArgumentException
...
- (mbgl::FeatureType)gcl_featureType {
    id value = self.constantValue;
    if ([value isKindOfClass:NSString.class]) {
...
- (std::vector<mbgl::FeatureIdentifier>)gcl_aggregateFeatureIdentifier {
    if ([self.constantValue isKindOfClass:[NSArray class]] || [self.constantValue isKindOfClass:[NSSet class]]) {
        ...
        for (id value in self.constantValue) {
            ...
            convertedValues.push_back(expression.gcl_featureIdentifier);
        }
        return convertedValues;
    }
    [NSException raise:NSInvalidArgumentException
...
- (mbgl::FeatureIdentifier)gcl_featureIdentifier {
    mbgl::Value mbglValue = self.gcl_constantMBGLValue;
