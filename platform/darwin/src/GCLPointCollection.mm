...
@implementation GCLPointCollection
{
...
- (nullable instancetype)initWithCoder:(NSCoder *)decoder {
    if (self = [super initWithCoder:decoder]) {
        ...
        _coordinates = [coordinates gcl_coordinates];
    }
    return self;
}
...
- (void)encodeWithCoder:(NSCoder *)coder {
    ...
    [coder encodeObject:[NSArray gcl_coordinatesFromCoordinates:_coordinates] forKey:@"coordinates"];
}

- (BOOL)isEqual:(id)other {
    if (self == other) return YES;
    if (![other isKindOfClass:[GCLPointCollection class]]) return NO;

    GCLPointCollection *otherCollection = (GCLPointCollection *)other;
...
- (GCLCoordinateBounds)overlayBounds {
    if (!_bounds) {
        ...
        for (auto coordinate : _coordinates) {
            ...
            bounds.extend(GCLLatLngFromLocationCoordinate2D(coordinate));
        }
        _bounds = bounds;
    }
    return GCLCoordinateBoundsFromLatLngBounds(*_bounds);
}
...
{
    return GCLCoordinateBoundsIntersectsCoordinateBounds(self.overlayBounds, overlayBounds);
}
