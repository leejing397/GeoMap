...
@class GCLOfflinePack;
@protocol GCLOfflineRegion;
@protocol GCLOfflineStorageDelegate;

/**
 Posted by the shared `GCLOfflineStorage` object when an `GCLOfflinePack`
 ...
 The `object` is the `GCLOfflinePack` object whose progress changed. The
 `userInfo` dictionary contains the pack’s current state in the
 `GCLOfflinePackStateUserInfoKey` key and details about the pack’s current
 progress in the `GCLOfflinePackProgressUserInfoKey` key. You may also consult
 the `GCLOfflinePack.state` and `GCLOfflinePack.progress` properties, which
 ...
 path.
 */
extern GCL_EXPORT const NSNotificationName GCLOfflinePackProgressChangedNotification;

/**
 Posted by the shared `OfflineStorage` object whenever an `GCLOfflinePack`
 ...
 The `object` is the `GCLOfflinePack` object that encountered the error. The
 `userInfo` dictionary contains the error object in the
 `OfflinePackErrorUserInfoKey` key.
 */
extern GCL_EXPORT const NSNotificationName GCLOfflinePackErrorNotification;

/**
 Posted by the shared `OfflineStorage` object when the maximum number of
 Mapbox-hosted tiles has been downloaded and stored on the current device.

 The `object` is the `OfflinePack` object that reached the tile limit in the
 course of downloading. The `userInfo` dictionary contains the tile limit in the
 `OfflinePackMaximumCountUserInfoKey` key.

 Once this limit is reached, no instance of `OfflinePack` can download
 additional tiles from Mapbox APIs until already downloaded tiles are removed by
 calling the `-[OfflineStorage removePack:withCompletionHandler:]` method.
 Contact your Mapbox sales representative to have the limit raised.
 */
extern GCL_EXPORT const NSNotificationName GCLOfflinePackMaximumMapboxTilesReachedNotification;

/**
 A key in the `userInfo` property of a notification posted by `GCLOfflinePack`.
 */
typedef NSString *GCLOfflinePackUserInfoKey NS_EXTENSIBLE_STRING_ENUM;
...
/**
 ...
 `OfflinePackProgressChangedNotification` notification. Call `-integerValue`
 on the object to receive the `OfflinePackState`-typed state.
 */
extern GCL_EXPORT const GCLOfflinePackUserInfoKey GCLOfflinePackUserInfoKeyState;

extern GCL_EXPORT NSString * const GCLOfflinePackStateUserInfoKey __attribute__((deprecated("Use MGLOfflinePackUserInfoKeyState")));
...
/**
 ...
 `OfflinePackProgressChangedNotification` notification. Call
 `-OfflinePackProgressValue` on the object to receive the
 `OfflinePackProgress`-typed progress.
 */
extern GCL_EXPORT const GCLOfflinePackUserInfoKey GCLOfflinePackUserInfoKeyProgress;

extern GCL_EXPORT NSString * const GCLOfflinePackProgressUserInfoKey __attribute__((deprecated("Use MGLOfflinePackUserInfoKeyProgress")));
...
/**
 ...
 an `OfflinePackErrorNotification` notification. The error’s domain is
 `ErrorDomain`. See `ErrorCode` for possible error codes.
 */
extern GCL_EXPORT const GCLOfflinePackUserInfoKey GCLOfflinePackUserInfoKeyError;

extern GCL_EXPORT NSString * const GCLOfflinePackErrorUserInfoKey __attribute__((deprecated("Use MGLOfflinePackUserInfoKeyError")));
...
/**
 ...
 `OfflinePackMaximumMapboxTilesReachedNotification` notification. Call
 `-unsignedLongLongValue` on the object to receive the `uint64_t`-typed tile
 limit.
 */
extern GCL_EXPORT const GCLOfflinePackUserInfoKey GCLOfflinePackUserInfoKeyMaximumCount;

extern GCL_EXPORT NSString * const GCLOfflinePackMaximumCountUserInfoKey __attribute__((deprecated("Use MGLOfflinePackUserInfoKeyMaximumCount")));
 ...
 */
typedef void (^GCLOfflinePackAdditionCompletionHandler)(GCLOfflinePack * _Nullable pack, NSError * _Nullable error);
 ...
 */
typedef void (^GCLOfflinePackRemovalCompletionHandler)(NSError * _Nullable error);

/**
 The type of resource that is requested.
 */
typedef NS_ENUM(NSUInteger, GCLResourceKind) {
    /** Unknown type */
    GCLResourceKindUnknown,
    /** Style sheet JSON file */
    GCLResourceKindStyle,
    /** TileJSON file as specified in https://www.mapbox.com/mapbox-gl-js/style-spec/#root-sources */
    GCLResourceKindSource,
    ...
    GCLResourceKindSpriteJSON,
    /** Image data for a georeferenced image source. **/
    GCLResourceKindImage,
};

/**
 OfflineStorage implements a singleton (shared object) that manages offline
 ...
 */
GCL_EXPORT
@interface GCLOfflineStorage : NSObject
...
/**
 ...
 or endpoints.
 */
@property(nonatomic, weak, nullable) IBOutlet id<GCLOfflineStorageDelegate> delegate;
 ...
 */
@property (nonatomic, strong, readonly, nullable) NS_ARRAY_OF(GCLOfflinePack *) *packs;
...
/**
 ...
 The pack has an initial state of `GCLOfflinePackStateInactive`. To begin
 downloading resources, call `-[GCLOfflinePack resume]` on the pack from within
 the completion handler. To monitor download progress, add an observer for
 `GCLOfflinePackProgressChangedNotification`s about that pack.
 ...
 */
- (void)addPackForRegion:(id <GCLOfflineRegion>)region withContext:(NSData *)context completionHandler:(nullable GCLOfflinePackAdditionCompletionHandler)completion;
 ...
 */
- (void)removePack:(GCLOfflinePack *)pack withCompletionHandler:(nullable GCLOfflinePackRemovalCompletionHandler)completion;

/**
 Forcibly, asynchronously reloads the `packs` property. At some point after this
 method is called, the pointer values of the `OfflinePack` objects in the
...
/**
 ...
 `OfflinePackMaximumMapboxTilesReachedNotification` is posted for every
...
/**
 ...
 as part of an offline pack or due to caching during normal use of `GCLMapView`.
 */
...
/**
 The `OfflineStorageDelegate` protocol defines methods that a delegate of an
 `OfflineStorage` object can optionally implement to transform various types
 of URLs before downloading them via the internet.
 */
@protocol GCLOfflineStorageDelegate <NSObject>
