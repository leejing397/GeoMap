...
/**
 The `GCLMultiPoint` class is an abstract superclass used to define shapes
 composed of multiple vertices.

 Create instances of `GCLPolyline` or `GCLPolygon` in order to use 
 properties of `GCLMultiPoint`. Do not create instances of `GCLMultiPoint`
 ...
 Do not confuse `GCLMultiPoint` with `GCLPointCollection`, which represents a
 collection of related but disconnected points.
 */
GCL_EXPORT
@interface GCLMultiPoint : GCLShape
...
/**
 ...
 immediately. If the shape is part of an `GCLShapeSource` object, you must
 explicitly set the `GCLShapeSource.shape` property in order for any style
