...
/**
 ...
 This option is only applicable to `GCLRasterSource` objects; it is ignored when
 initializing `GCLVectorSource` objects.
 */
extern GCL_EXPORT const GCLTileSourceOption GCLTileSourceOptionTileSize;

/**
 `GCLRasterSource` is a map content source that supplies raster image tiles to
 ...
 A raster source is added to an `GCLStyle` object along with one or more
 ...
 `GCLRasterSource` object that you can use to initialize new style layers. You
 ...
 ```swift
 let source = GCLRasterSource(identifier: "clouds", tileURLTemplates: ["https://example.com/raster-tiles/{z}/{x}/{y}.png"], options: [
     .minimumZoomLevel: 9,
     .maximumZoomLevel: 16,
     .tileSize: 512,
     .attributionInfos: [
 ...
 */
GCL_EXPORT
@interface GCLRasterSource : GCLTileSource
...
/**
 ...
 `GCLTileSourceOptionTileSize` documentation for more information about tile
...
/**
 ...
 @param tileSize The width and height (measured in points) of each tiled image
    in the raster source. See the `GCLTileSourceOptionTileSize` documentation
    for details.
 @return An initialized raster source.
 */
